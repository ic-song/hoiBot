...
- Treat a Notion planning document as the source of product intent, user-visible behavior, and operational policy, not as implementation authority. Planning documents may have been drafted with generative AI.
...
- Ignore generated implementation details such as suggested variable names, helper names, JSON shapes, storage paths, locking primitives, save order, or code structure unless the user explicitly confirms that detail as a requirement.
...
- Do not block development merely because a planning document's generated implementation examples do not match the repository. Block only when product intent is materially ambiguous, such as conflicting formulas, unresolved reward eligibility, user-visible policy, or operational behavior that would change the result.
- When the current code establishes a safe precedent for a missing implementation detail, follow that precedent and record the resulting user-visible planning decision in Notion instead of asking the planner to design internal code.
...
- The required order is:
  ...
  10. Synchronize the source Notion document's planning sections with the actual validated behavior. Keep purpose, command syntax, formulas, limits, rewards, UI, exceptions, acceptance criteria, and operational policy current; remove or correct generated examples that conflict with the implemented formula. Do not turn the planning document into an internal variable or storage-schema document.
  11. Update `COMMAND_INDEX.md`, `COMMAND_REGISTRY.md`, changelog/version files, or other docs only when the rules require synchronization.
  12. Report the Notion document checked and synchronized, changed files, validation result, and remaining risks.
...
- If no matching Notion page exists, create one in the planning database as part of the production-reflection workflow. Record the actual product intent and validated behavior: purpose, affected commands, user-visible UI, formulas and limits, rewards or costs, exception handling, acceptance criteria, and operational notes. Keep internal variable names, helper names, and storage schemas out unless they are independently required for operations.
- A newly created record for an unreflected production change starts as `상태 = 🛠 READY`; keep `운영 반영예정일` blank when immediate reflection is intended. After `origin/feature/prod` is verified, update it through the same required operation to `상태 = 🧪 DEV`, the KST `운영반영일`, and `운영반영버전 = ver_<HoiBotVersion>`.
