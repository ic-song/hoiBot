...
const feedTriggerStart = main.indexOf('if (feedCount >= 100000) recordAdventureQuestAction(data, sender, 7, "feed100000", 0);');
assert(feedTriggerStart >= 0, "7단계는 정상 사용한 먹이 10만 개 이상을 인정");
const feedTrigger = main.slice(feedTriggerStart, main.indexOf(";", feedTriggerStart) + 1);
vm.runInContext("function recordFeedCommand(feedCount) { " + feedTrigger + " }", context);
assert.strictEqual(context.getAdventureQuestCompletionCheck(data, {}, {}, home, {}, {}, user).complete, false);
context.recordFeedCommand(99999);
assert.strictEqual(context.getAdventureQuestCompletionCheck(data, {}, {}, home, {}, {}, user).complete, false, "10만 개 미만은 미완료");
context.recordFeedCommand(100000);
assert.strictEqual(context.getAdventureQuestCompletionCheck(data, {}, {}, home, {}, {}, user).complete, true, "정확히 10만 개 사용 인정");
state.progress = {};
context.recordFeedCommand(100001);
assert.strictEqual(context.getAdventureQuestCompletionCheck(data, {}, {}, home, {}, {}, user).complete, true, "10만 개 초과 사용도 인정");
assert(context.getAdventureQuestGoalLines(state, { complete: true }, false)[0].includes("100,000개 이상"));
