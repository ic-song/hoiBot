...
echo [3/3] /호이랜드 전체를 data로 ADB pull
echo ------------------------------------------------------------
move "%LOCAL_DATA_DIR%" "%ROLLBACK_DIR%" > nul 2>&1
if errorlevel 1 goto FAIL_ROLLBACK

"%ADB_EXE%" -s "%TARGET_ADB_DEVICE%" pull "%REMOTE_DATA_DIR%" "%LOCAL_DATA_DIR%"
if errorlevel 1 goto FAIL_PULL_RESTORE
if not exist "%LOCAL_DATA_DIR%\" goto FAIL_PULL_RESTORE

for /f %%c in ('powershell -NoProfile -Command "$files=@(Get-ChildItem -LiteralPath $env:LOCAL_DATA_DIR -File -Recurse); Write-Output $files.Count"') do set "PULL_COUNT=%%c"
if not defined PULL_COUNT goto FAIL_PULL_RESTORE
if "%PULL_COUNT%"=="0" goto FAIL_PULL_RESTORE
