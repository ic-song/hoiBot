...
for (const name of ["getAdventureStarterRecoveryStatus", "getAdventureStarterRecoveryPlan", "applyAdventureStarterRecovery", "buildAdventureStarterRecoveryPreviewMessages"]) {
    vm.runInContext(extractFunction(name), context);
}
const commandStart = main.indexOf("        var starterRecoveryExecute = ");
...
assert(main.includes('/^\\/스타터중복회수 실행 [1-9]\\d* 확인$/.test(command)'));
vm.runInContext("function runRecoveryCommand() {" + command + "}", context);

context.runRecoveryCommand();
assert(replies[0].includes("회수 가능: 1명"));
assert(replies[0].includes("잔액 부족 보류: 1명"));
assert(replies.join("\n").includes("회수가능"));
assert.strictEqual(saves, 0);
replies.length = 0;
context.room = "일반방";
context.msg = "/스타터중복회수 실행 2 확인";
context.runRecoveryCommand();
...
assert.strictEqual(replies.length, 0);
context.sender = "관리자";
context.msg = "/스타터중복회수 실행 3 확인";
context.runRecoveryCommand();
assert(replies[0].includes("대상자 수가 변경"));
assert.strictEqual(saves, 0);
replies.length = 0;
context.msg = "/스타터중복회수 실행 2 확인";
context.runRecoveryCommand();
assert.strictEqual(saves, 1);
assert(replies[0].includes("회수 완료: 1명"));
assert.strictEqual(members["회수가능"].point, 0);
...
assert.strictEqual(members["잔액부족"].point, starter.points);
assert.strictEqual(members["잔액부족"].bag["땅문서📜"], 19);
assert.strictEqual(members["잔액부족"].adventureQuest.receipts.starterMemberRewardsRecovered, undefined);
replies.length = 0;
context.msg = "/스타터중복회수 실행 2 확인";
context.runRecoveryCommand();
