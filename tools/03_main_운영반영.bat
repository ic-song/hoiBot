...
if /i "%TARGET_LD_INDEX%"=="auto" (
	...
	for /f "tokens=1 delims=," %%i in ('"%LD_CONSOLE_EXE%" list2') do (
		"%LD_CONSOLE_EXE%" adb --index %%i --command "shell find /storage/emulated/0 -path '*/Bots/main/main.js' -type f 2>/dev/null" > "%DEPLOY_LOG%" 2>&1
		for /f "usebackq delims=" %%p in ("%DEPLOY_LOG%") do (
			echo %%p | findstr /i /c:"/Bots/main/main.js" > nul 2>&1
			if not errorlevel 1 (
...
echo  검색 기준 = /storage/emulated/0/*/Bots/main/main.js
