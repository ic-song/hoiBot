...
5. 관리 홈페이지 목업의 임시 캐릭터·통계 데이터를 실제 회원 검색·ProfileView·감사 API에 연결하고 로딩·빈 결과·오류 상태를 구현한다.
6. 구현된 domain Service를 legacy 명령별 golden fixture와 대조한 뒤 Iris·Discord·관리 API adapter에 수직 기능 단위로 연결한다.
7. 다른 참여자 이벤트, WebSocket, 다중 이미지/파일/반응 등 기존 미검증 Iris 입력을 별도 테스트방에서 계속 검증한다.
8. Windows Docker 검증 결과를 Ubuntu Docker 환경에서도 동일 image digest·migration·backup/restore 절차로 재현한다.
9. 전체 기능과 운영 snapshot reconciliation을 통과한 뒤 최종 일괄 전환 점검표를 실행한다.
