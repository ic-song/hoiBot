...
- 최신 자동 검증은 runtime 테스트 158개와 runtime typecheck/build, `main.js`·`Info.js` 구문 검사를 통과했다. 길드 가입·가입조건·강제제명 통합 rehearsal은 migration 33개, 71문장 합성 fixture 2회 적용·verify-only, 대표 35개 테이블을 확인했다. 가입은 membership 1·티켓 0·완료 request, 가입조건은 12,345 EXP, 강제제명은 대상 membership 0을 확인했다. MariaDB 재시작 후 같은 event를 다시 실행해 결과가 같고 각 operation/execution/audit/outbox 효과가 한 번만 남는 것도 확인했다. 운영 JSON과 운영 DB는 건드리지 않았다.
