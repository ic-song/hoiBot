...
export class PetTitleCanonicalMutationProvider {
  ...
  async adminSync(database:AppWiringMutationParticipant,claim:AppWiringClaim,input:{actor:string}):Promise<PetTitleCanonicalBatchResult>{
    void database;void claim;void input;
    throw new Error("PET_TITLE_ACTIVE_MEMBER_AUTHORITY_REQUIRED");
  }
  ...
  private async adminBatch(database:AppWiringMutationParticipant,claim:AppWiringClaim,actor:string,operationType:"ADMIN_SYNC"|"ADMIN_RESET"):Promise<PetTitleCanonicalBatchResult>{
    if(claim.route!=="MODERN"||claim.effectMode!=="MUTATION")throw new Error("PET_TITLE_APP_WIRING_MUTATION_CLAIM_REQUIRED");
    await lockPetTitleGlobalScope(database);
    ...
    const rows=await database.query<BatchOwnedTitleRow[]>(`SELECT owned.owned_pet_title_id,owned.player_id,owned.acquisition_sequence,CASE WHEN selection.owned_pet_title_id IS NULL THEN 0 ELSE 1 END selected_flag
