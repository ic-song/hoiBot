...
type AccessEvidence=PrivateAccessEvidence|{readonly mode:"OPEN_GROUP"};

async function assertPrivatePassAccess(transaction:AppWiringReadParticipant,externalUserId:string):Promise<PrivateAccessEvidence>{
  ...
  if(actor===undefined||actor.identity_status!=="linked"||actor.player_status!=="active")throw new Error("PET_SKILL_INFO_PRIVATE_IDENTITY_REQUIRED");
  ...
  if(passes.length===0)throw new Error("PET_SKILL_INFO_PRIVATE_PASS_REQUIRED");
...
function assertFormalReceiptProjection(projection:unknown,input:{command:PetSkillInfoIngressCommand;environment:VerifiedEnvironmentContext;event:NormalizedIrisEvent;replyIdentity:NormalizedIrisEvent;channelType:"open_group"|"open_direct"}):void{
  ...
  if(receipt.version!==RECEIPT_VERSION||typeof binding!=="object"||binding===null||Array.isArray(binding)||!("value" in receipt)||typeof receipt.authorization!=="object"||receipt.authorization===null)throw new Error("PET_SKILL_INFO_RECEIPT_PROJECTION_INVALID");
...
export async function executePetSkillInfoReadOnlyRecovery(input:{
  ...
  const recovered=await input.recovery.execute({
    ...
    evaluateInSnapshot:async transaction=>{
      ...
      const authorization:AccessEvidence=input.channelType==="open_direct"?await assertPrivatePassAccess(transaction,input.replyIdentity.userId!):{mode:"OPEN_GROUP"};
