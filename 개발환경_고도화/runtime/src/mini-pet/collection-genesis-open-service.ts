...
export interface CollectionGenesisOpenResult {
...
// 인자나 접미사 없는 컬렉션 창세 패키지 명령만 허용합니다.
export function isCollectionGenesisOpenCommand(message: string | undefined): boolean {
  return message === "/컬렉션창세오픈";
}
...
function stored(value: string | CollectionGenesisOpenResult): CollectionGenesisOpenResult {
  return typeof value === "string" ? JSON.parse(value) as CollectionGenesisOpenResult : value;
}
...
// 컬렉션 창세 패키지를 소비하고 미니펫과 뽑기권을 원자적으로 지급합니다.
export class CollectionGenesisOpenService {
  constructor(private readonly database: DatabaseClient) {}

  async handle(command: CollectionGenesisOpenCommand): Promise<CollectionGenesisOpenResult> {
    if (!isCollectionGenesisOpenCommand(command.message)) {
      throw new ApplicationError("INVALID_COLLECTION_GENESIS_OPEN_COMMAND", "정확한 /컬렉션창세오픈을 입력해주세요.", 422);
    }
    ...
    return this.database.withTransaction(async (tx) => {
      ...
      const scope = `mini-pet.collection-genesis-open:${owner.identity_id}`;
      const key = eventKey(command.eventId);
      const prior = await tx.query<Array<{ result_json: string | CollectionGenesisOpenResult | null }>>(
      ...
      if (packageItem === undefined || packageItem.version === null || packageQuantity < 1n) {
        throw new ApplicationError(
          "COLLECTION_GENESIS_PACKAGE_REQUIRED",
          `❌ [${ranked(owner)}] 님\n[${PACKAGE_NAME}] 아이템이 없습니다.\n(보유: ${packageQuantity}개)`, 409
        );
      }
      ...
      if (packageWrite.affectedRows !== 1n) {
        throw new ApplicationError("COLLECTION_GENESIS_OPEN_CONFLICT", "아이템 정보가 먼저 변경되었습니다.", 409);
      }

      const ticketAfter = (ticketItem.quantity ?? 0n) + TICKET_REWARD;
      const ticketWrite = ticketItem.version === null
        ? await tx.execute("INSERT INTO inventory_stacks (player_id, item_id, quantity, version) VALUES (?, ?, ?, 1)", [owner.player_id, ticketItem.item_id, ticketAfter])
        : await tx.execute(
          "UPDATE inventory_stacks SET quantity = ?, version = version + 1 WHERE player_id = ? AND item_id = ? AND version = ?",
      ...
      await tx.execute(
        "INSERT INTO inventory_ledger (operation_id, sequence_no, player_id, item_id, quantity_delta, reason_code) VALUES (?, 1, ?, ?, -1, 'collection_genesis_package_open'), (?, 2, ?, ?, ?, 'collection_genesis_ticket_reward')",
      ...
      await tx.execute(
        "INSERT INTO command_executions (event_id, command_code, operation_id, execution_status, result_code, created_at, completed_at) VALUES (?, 'collection_genesis_open', ?, 'completed', 'reply_queued', UTC_TIMESTAMP(3), UTC_TIMESTAMP(3))",
        [command.eventId, operation.insertId]
      );
      const audit = await tx.execute(
        "INSERT INTO command_audit (operation_id, actor_type, actor_id, target_type, target_id, action_code, result_code, reason, change_summary_json, created_at) VALUES (?, 'external_identity', ?, 'player', ?, 'mini_pet.collection_genesis_open', 'success', 'Iris /컬렉션창세오픈', ?, UTC_TIMESTAMP(3))",
      ...
      const result: CollectionGenesisOpenResult = {
        status: "opened", playerId: owner.player_id.toString(), packageQuantity: packageAfter.toString(),
        ticketQuantity: ticketAfter.toString(), ownedMiniPetId: ownedMiniPet.insertId.toString(),
