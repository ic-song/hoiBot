...
interface OwnedMiniPetRow {
  id: bigint; player_id: bigint; mini_pet_definition_id: bigint; custom_name: string | null; custom_emoji: string | null;
...
function petLabel(row: OwnedMiniPetRow): string { const name=row.custom_name??row.display_name,emoji=row.custom_emoji??row.emoji_value??"",grade=row.grade_display_name??row.grade_code??"등급 없음"; return `${name}${emoji} [${grade}]`; }
function petSnapshot(row: OwnedMiniPetRow): Record<string, unknown> {
  return { ownedMiniPetId:row.id.toString(),miniPetDefinitionId:row.mini_pet_definition_id.toString(),customName:row.custom_name,displayName:row.display_name,emoji:row.custom_emoji??row.emoji_value,gradeCode:row.grade_code,gradeDisplayName:row.grade_display_name,progress:row.progress.toString(),enhancementLevel:row.enhancement_level.toString(),battleExperience:row.battle_experience.toString(),castleExperience:row.castle_experience.toString(),raidExperience:row.raid_experience.toString(),salePrice:row.sale_price?.toString()??null,elite:row.is_elite===1,bagSequence:row.bag_sequence?.toString()??null,version:row.version.toString() };
}
...
export class MiniPetAdminOwnedDeleteService {
  ...
  async handle(input:{eventId:string;externalUserId:string;destinationId:string;message:string}):Promise<MiniPetAdminOwnedDeleteResult>{
    ...
    return this.database.withTransaction(async transaction=>{
      ...
      const owned=await transaction.query<OwnedMiniPetRow[]>(`SELECT pet.id,pet.player_id,pet.mini_pet_definition_id,pet.custom_name,pet.custom_emoji,pet.progress,pet.enhancement_level,pet.battle_experience,pet.castle_experience,pet.raid_experience,pet.sale_price,pet.is_elite,pet.bag_sequence,pet.version,pet.equipped,definition.display_name,definition.grade_code,definition.grade_display_name,definition.emoji_value FROM owned_mini_pets pet JOIN mini_pet_definitions definition ON definition.id=pet.mini_pet_definition_id WHERE pet.player_id=? ORDER BY pet.bag_sequence IS NULL,pet.bag_sequence,pet.id FOR UPDATE`,[player.player_id]);
