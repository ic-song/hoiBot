...
export type MatzangCommand = "battle" | "rank";
export type MatzangCommandResult = { status: "battle" | "rank" | "wrong_room" | "inactive" | "ignored"; data: string; outboxId?: string };
...
export class MatzangCommandService {
  ...
  private async reply(input: { eventId: string; destinationId: string; playerId: string }, command: MatzangCommand, status: "rank" | "wrong_room" | "inactive", data: string): Promise<MatzangCommandResult> {
    return this.db.withTransaction(async (transaction) => this.persistReply(transaction, input, command, status, data));
  }

  private async persistReply(transaction: DatabaseTransaction, input: { eventId: string; destinationId: string; playerId: string }, command: MatzangCommand, status: "rank" | "wrong_room" | "inactive", data: string): Promise<MatzangCommandResult> {
    ...
    await transaction.execute("INSERT INTO command_executions(event_id,command_code,operation_id,execution_status,result_code,created_at,completed_at) VALUES (?,?,?,'completed',?,UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))", [input.eventId, command === "rank" ? "MATZZANG_RANK" : "MATZZANG_BATTLE", operation.insertId, status]);
