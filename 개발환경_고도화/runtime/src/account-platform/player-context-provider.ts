...
function normalizeSelfLocator(input: { identityProviderCode: string; externalUserId: string; externalContextId: string }): {
  ...
  if (input.externalUserId.length === 0 || input.externalUserId.length > 191
    || input.externalContextId.length === 0 || input.externalContextId.length > 191) fail("PLAYER_CONTEXT_LOCATOR_INVALID");
  if (providerCode === "kakao") return { providerCode, platformCode: "KAKAO", contextType: "ROOM", identityScopeKey: input.externalContextId };
  if (providerCode === "discord") return { providerCode, platformCode: "DISCORD", contextType: "SERVER", identityScopeKey: "PLATFORM_ACCOUNT" };
  return fail("PLAYER_CONTEXT_PROVIDER_UNSUPPORTED");
}

function collapsePlayerContext(rows: readonly PlayerContextRow[], input: {
  ...
  for (const row of rows) {
    if (row.legacy_player_id === null || row.external_identity_id === null || row.canonical_player_id === null
      ...
      || row.display_name === null || row.provider_code === null || !/^[a-z][a-z0-9]{7}$/.test(row.canonical_player_id)) fail("PLAYER_CONTEXT_MAPPING_DRIFT");
    ...
    } catch { return fail("PLAYER_CONTEXT_MAPPING_DRIFT"); }
...
function collapsePlayerTarget(rows: readonly PlayerContextRow[]): PlayerTarget {
  ...
  for (const row of rows) {
    ...
    let legacyPlayerId: bigint;
    try { legacyPlayerId = BigInt(row.legacy_player_id); }
    catch { return fail("PLAYER_CONTEXT_MAPPING_DRIFT"); }
  ...
  async resolveSelf(database: PlayerContextReadParticipant, input: {
    ...
    const activeRows = await database.query<PlayerContextRow[]>(
      `SELECT selection.active_player_id AS legacy_player_id,canonical_player.player_id AS canonical_player_id,
         ...
         LEFT JOIN external_identities selected_identity ON selected_identity.player_id=player.id AND selected_identity.status='linked'
    ...
    if (activeRows.length > 0) return collapsePlayerContext(activeRows, {
      selectionSource: "ACTIVE_CONTEXT", platformCode: locator.providerCode, externalContextId: input.externalContextId, requireCallerLink: true,
    });
    ...
    return collapsePlayerContext(legacyRows, {
      selectionSource: "LEGACY_CROSSWALK", platformCode: locator.providerCode, externalContextId: input.externalContextId,
    });
  }
