export type ContributionPassCommand = { action: "add"; target: string; option: "permanent" | "dated"; endDate: string | null; rawEndDate: string } | { action: "delete"; target: string };

const ADD = "/공헌패스추가";
const DELETE = "/공헌패스삭제";

// 공헌패스 추가·삭제 후보만 broad legacy guard와 동일하게 분류합니다.
export function isContributionPassCommandCandidate(message: string | undefined): boolean {
  return message !== undefined && /^\/공헌패스(?:추가|삭제),\s*.+$/.test(message);
}
...
export function parseContributionPassCommand(message: string): ContributionPassCommand | null {
  if (!isContributionPassCommandCandidate(message)) return null;
...
export function normalizeContributionPassDispatchMessage(message: string): string {
  const command = parseContributionPassCommand(message);
