...
import { isRandomBoxCraftCommand, RandomBoxCraftService } from "./crafting/random-box-craft-service.js";
import { FurnitureBoutiqueBoxCraftService, isFurnitureBoutiqueBoxCraftCommand } from "./crafting/furniture-boutique-box-craft-service.js";
...
export function buildApp(config: AppConfig, dependencies: AppDependencies = {}) {
  ...
  app.post<{ Body: IrisPayload; Querystring: TokenQuery }>(
    ...
    async (request, reply) => {
      ...
      if (isOperationalChannel && processing !== undefined && !processing.duplicate
        && isRandomBoxCraftCommand(normalizedEvent.message)
        && normalizedEvent.userId !== undefined && normalizedEvent.channelId !== undefined) {
        try {
          const result = await new RandomBoxCraftService(database!).handle({
            externalUserId: normalizedEvent.userId,
            channelId: normalizedEvent.channelId,
            message: normalizedEvent.message!,
            eventId: normalizedEvent.eventId
          ...
          if (error instanceof ApplicationError && [409, 422].includes(error.statusCode)) {
            processing.replies.push(await eventProcessor!.queueCommandReply(normalizedEvent, "random_box_craft", error.message));
          } else {
            throw error;
          }
        }
      ...
      if (isOperationalChannel && processing !== undefined && !processing.duplicate
        && isFurnitureBoutiqueBoxCraftCommand(normalizedEvent.message)
        && normalizedEvent.userId !== undefined && normalizedEvent.channelId !== undefined) {
        try {
          const result = await new FurnitureBoutiqueBoxCraftService(database!).handle({
            externalUserId: normalizedEvent.userId,
            channelId: normalizedEvent.channelId,
            message: normalizedEvent.message!,
            eventId: normalizedEvent.eventId
          ...
          if (error instanceof ApplicationError && [409, 422].includes(error.statusCode)) {
            processing.replies.push(await eventProcessor!.queueCommandReply(normalizedEvent, "furniture_boutique_box_craft", error.message));
          } else {
            throw error;
          }
        }
