...
import { isPetTitleSaleCommand, PetTitleSaleService } from "./pet/title-sale-service.js";
...
export function buildApp(config: AppConfig, dependencies: AppDependencies = {}) {
  ...
  app.post<{ Body: IrisPayload; Querystring: TokenQuery }>(
    ...
    async (request, reply) => {
      ...
      if (isOperationalChannel && processing !== undefined && !processing.duplicate
        && isMiniPetTitleSelectCommand(normalizedEvent.message)
        && normalizedEvent.userId !== undefined && normalizedEvent.channelId !== undefined
        && database !== undefined) {
        try {
          const result = await new MiniPetTitleSelectService(database).execute({
            externalUserId: normalizedEvent.userId,
            channelId: normalizedEvent.channelId,
            eventId: normalizedEvent.eventId,
            message: normalizedEvent.message ?? "",
          ...
          if (result.status === "selected") {
            ...
            processing.replies.push(await eventProcessor!.queueCommandReply(normalizedEvent, "mini_pet_title_select", result.data));
          }
        } catch (error) {
          if (error instanceof ApplicationError && [409, 422].includes(error.statusCode)) {
      ...
      if (isOperationalChannel && processing !== undefined && !processing.duplicate
        && isPetTitleSaleCommand(normalizedEvent.message)
        && normalizedEvent.userId !== undefined && normalizedEvent.channelId !== undefined
        && database !== undefined) {
        try {
          const result = await new PetTitleSaleService(database).execute({
            externalUserId: normalizedEvent.userId,
            channelId: normalizedEvent.channelId,
            eventId: normalizedEvent.eventId,
            message: normalizedEvent.message ?? "",
          ...
          if (result.status === "sold") {
            ...
            processing.replies.push(await eventProcessor!.queueCommandReply(normalizedEvent, "pet_title_sale", result.data));
          }
        } catch (error) {
          if (error instanceof ApplicationError && [409, 422].includes(error.statusCode)) {
            processing.replies.push(await eventProcessor!.queueCommandReply(normalizedEvent, "pet_title_sale_error", error.message));
          } else {
            throw error;
          }
        }
      ...
      if (isOperationalChannel && processing !== undefined && !processing.duplicate
        && isTerritoryProtectionGrantCommand(normalizedEvent.message)
        && normalizedEvent.userId !== undefined && normalizedEvent.channelId !== undefined
        && database !== undefined) {
        try {
          const result = await new TerritoryProtectionGrantService(database).execute({
            externalUserId: normalizedEvent.userId,
            channelId: normalizedEvent.channelId,
            eventId: normalizedEvent.eventId,
            message: normalizedEvent.message ?? ""
          ...
          if (error instanceof ApplicationError && error.statusCode === 403) {
            // 레거시 최고관리자 전용 명령은 권한이 없으면 응답하지 않습니다.
          } else if (error instanceof ApplicationError && [404, 409, 422].includes(error.statusCode)) {
            processing.replies.push(await eventProcessor!.queueCommandReply(normalizedEvent, "territory_protection_grant_error", error.message));
          } else {
            throw error;
          }
        }
      ...
      if (isOperationalChannel && processing !== undefined && !processing.duplicate
        && isFixedItemGrantCommand(normalizedEvent.message)
        && normalizedEvent.userId !== undefined && normalizedEvent.channelId !== undefined
        && database !== undefined) {
        try {
          const result = await new FixedItemGrantService(database).execute({ externalUserId: normalizedEvent.userId,
            channelId: normalizedEvent.channelId, eventId: normalizedEvent.eventId, message: normalizedEvent.message ?? "" });
          processing.replies.push({ outboxId: result.outboxId, room: normalizedEvent.channelId, data: result.data });
        } catch (error) {
          if (error instanceof ApplicationError && error.statusCode === 403) {
            // 레거시 총괄 운영자 전용 지급 명령은 권한이 없으면 응답하지 않습니다.
          } else if (error instanceof ApplicationError && [404, 409, 422].includes(error.statusCode)) {
            processing.replies.push(await eventProcessor!.queueCommandReply(normalizedEvent, "fixed_item_grant_error", error.message));
          } else {
            throw error;
          }
        }
      ...
      if (isOperationalChannel && processing !== undefined && !processing.duplicate
        && isBoosterDualGrantCommand(normalizedEvent.message)
        && normalizedEvent.userId !== undefined && normalizedEvent.channelId !== undefined
        && database !== undefined) {
        try {
          const result = await new BoosterDualGrantService(database).execute({ externalUserId: normalizedEvent.userId,
            channelId: normalizedEvent.channelId, eventId: normalizedEvent.eventId, message: normalizedEvent.message ?? "" });
          processing.replies.push({ outboxId: result.outboxId, room: normalizedEvent.channelId, data: result.data });
        } catch (error) {
          if (error instanceof ApplicationError && error.statusCode === 403) {
      ...
      if (isOperationalChannel && processing !== undefined && !processing.duplicate
        && isAdminRoleDeleteCommand(normalizedEvent.message)
        && normalizedEvent.userId !== undefined && normalizedEvent.channelId !== undefined
        && database !== undefined) {
        try {
          const result = await new AdminRoleDeleteService(database).execute({ externalUserId: normalizedEvent.userId,
            channelId: normalizedEvent.channelId, eventId: normalizedEvent.eventId, message: normalizedEvent.message ?? "" });
          processing.replies.push({ outboxId: result.outboxId, room: normalizedEvent.channelId, data: result.data });
        } catch (error) {
          if (error instanceof ApplicationError && error.statusCode === 403) {
            // 레거시 총괄 운영자 전용 관리자 삭제는 권한이 없으면 응답하지 않습니다.
          } else {
            const commandError = error instanceof Error ? error : new Error("관리자 삭제에 실패했습니다.");
            processing.replies.push({ outboxId: `error:${normalizedEvent.eventId}:admin-role-delete`, room: normalizedEvent.channelId, data: commandError.message });
          }
        }
      }

      if (isOperationalChannel && processing !== undefined && !processing.duplicate
        && isAdminRoleAddCommand(normalizedEvent.message)
        && normalizedEvent.userId !== undefined && normalizedEvent.channelId !== undefined
        && database !== undefined) {
        try {
          const result = await new AdminRoleAddService(database).execute({ externalUserId: normalizedEvent.userId,
            channelId: normalizedEvent.channelId, eventId: normalizedEvent.eventId, message: normalizedEvent.message ?? "" });
          processing.replies.push({ outboxId: result.outboxId, room: normalizedEvent.channelId, data: result.data });
        } catch (error) {
          if (error instanceof ApplicationError && error.statusCode === 403) {
            // 레거시 총괄 운영자 전용 관리자 추가는 권한이 없으면 응답하지 않습니다.
          } else {
            const commandError = error instanceof Error ? error : new Error("관리자 추가에 실패했습니다.");
            processing.replies.push({ outboxId: `error:${normalizedEvent.eventId}:admin-role-add`, room: normalizedEvent.channelId, data: commandError.message });
          }
        }
      }
      ...
      if (isOperationalChannel && processing !== undefined && !processing.duplicate
        && isMiniPetBulkSaleCommand(normalizedEvent.message)
        && normalizedEvent.userId !== undefined && normalizedEvent.channelId !== undefined
        && database !== undefined) {
        await new MiniPetBulkSaleService(database).execute({
          externalUserId: normalizedEvent.userId,
          channelId: normalizedEvent.channelId,
          eventId: normalizedEvent.eventId,
          message: normalizedEvent.message ?? "",
