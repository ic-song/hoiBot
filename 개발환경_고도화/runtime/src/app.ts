...
import { isPointBoxOpenCommand, PointBoxOpenService } from "./inventory/point-box-open-service.js";
...
export function buildApp(config: AppConfig, dependencies: AppDependencies = {}) {
  ...
  app.post<{ Body: IrisPayload; Querystring: TokenQuery }>(
    ...
    async (request, reply) => {
      ...
      if (isOperationalChannel && processing !== undefined && !processing.duplicate
        && isPointBoxOpenCommand(normalizedEvent.message)
        && normalizedEvent.userId !== undefined && normalizedEvent.channelId !== undefined) {
        try {
          const result = await new PointBoxOpenService(database!).handle({ externalUserId: normalizedEvent.userId, channelId: normalizedEvent.channelId, message: normalizedEvent.message!, eventId: normalizedEvent.eventId });
          ...
          if (error instanceof ApplicationError && [409, 422].includes(error.statusCode)) processing.replies.push(await eventProcessor!.queueCommandReply(normalizedEvent, "point_box_open", error.message));
          else throw error;
        }
      }
