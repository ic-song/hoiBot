...
export interface AdminGlobalGiftResult { readonly [key:string]:RequestReuseValue; readonly status:"completed"; readonly affectedMemberCount:string; readonly itemId:string; readonly quantityDelta:"1"; readonly replies:readonly AdminGlobalGiftReply[]; }
interface Context { readonly tx:DatabaseTransaction; readonly operationId?:string; }
interface OperationRow { admin_global_gift_operation_id:string; operation_key:string; operation_status:string; terminal_json:string|null; recipient_count:bigint|string; channel_count:number; result_fingerprint:string|null; announcement_fingerprint:string; }
...
class MariaGlobalGiftRfaStore implements TransactionReceiptOutboxStore<Context,AdminGlobalGiftResult>{
  ...
  private async readVerifiedTerminal(queryer:Pick<DatabaseTransaction,"query">,requestKey:string,lock:boolean):Promise<{operationId:string;terminal:TransactionReceiptOutboxTerminal<AdminGlobalGiftResult>}|undefined>{
    ...
    const row=(await queryer.query<OperationRow[]>(`SELECT admin_global_gift_operation_id,operation_key,operation_status,terminal_json,recipient_count,channel_count,result_fingerprint,announcement_fingerprint FROM canonical_admin_global_gift_operations WHERE request_key=?${completed}${locked}`,[requestKey]))[0];
    if(row===undefined)return undefined;
    if(row.operation_status!=="COMPLETED"||row.terminal_json===null)throw new Error("ADMIN_GLOBAL_GIFT_INCOMPLETE_RECEIPT");
    const operationId=row.admin_global_gift_operation_id,recipients=(await queryer.query<Array<{count_value:bigint|string}>>("SELECT COUNT(*) count_value FROM canonical_admin_global_gift_recipients WHERE admin_global_gift_operation_id=?",[operationId]))[0]!;
    ...
    if(BigInt(recipients.count_value)!==BigInt(row.recipient_count)||channels.length!==11||outboxes.length!==11||row.channel_count!==11)throw new Error("ADMIN_GLOBAL_GIFT_REPLAY_EVIDENCE_DRIFT");
    ...
    if(row.result_fingerprint!==terminal.typedReceipt.resultFingerprint||row.announcement_fingerprint!==sha256(ADMIN_GLOBAL_GIFT_MESSAGE)||terminal.typedReceipt.receiptId!==operationId||terminal.deliveryEvidence.deliveryKind!=="OUTBOX"||terminal.deliveryEvidence.outboxId!==BigInt(outboxes[0]!.outbox_message_id).toString()||terminal.deliveryEvidence.payloadFingerprint!==aggregate)throw new Error("ADMIN_GLOBAL_GIFT_REPLAY_TERMINAL_DRIFT");
...
export class AdminGlobalGiftService{
  ...
  async handle(input:{eventId:string;externalUserId:string;channelId:string;message:string}):Promise<AdminGlobalGiftResult&{readonly replayed:boolean}>{
    ...
    const executed=await this.rfa02.execute(request,store,async context=>{
      ...
      const result:AdminGlobalGiftResult={status:"completed",affectedMemberCount:players.length.toString(),itemId,quantityDelta:"1",replies},fingerprint=createRequestReuseTerminalReceipt(request,result).resultFingerprint;
      await tx.execute("UPDATE canonical_admin_global_gift_operations SET operation_status='STAGED',result_fingerprint=?,UPDATE_USER=?,UPDATE_TIME=? WHERE admin_global_gift_operation_id=? AND operation_status='PROCESSING'",[fingerprint,audit.UPDATE_USER,audit.UPDATE_TIME,operationId]);
