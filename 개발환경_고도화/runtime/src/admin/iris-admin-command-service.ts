...
import { isMemberVoiceAuthRewardCommandCandidate, MemberVoiceAuthRewardService } from "./member-voice-auth-reward-service.js";
...
import { isRocketPackageGrantCommandCandidate, RocketPackageGrantService } from "./rocket-package-grant-service.js";
import { isSlotNewbiePackageGrantCommandCandidate, SlotNewbiePackageGrantService } from "./slot-newbie-package-grant-service.js";
...
import { GuildTerritoryBoosterContributeService, isGuildTerritoryBoosterContributeCandidate } from "../guild/guild-territory-booster-contribute-service.js";
...
import { GuildCreateService, isGuildCreateCandidate } from "../guild/guild-create-service.js";
...
import { GuildRankRewardPayoutService, isGuildRankRewardPayoutCommand } from "../guild/guild-rank-reward-payout-service.js";
import { OverallRankRewardPayoutService, isOverallRankRewardPayoutCommand } from "./overall-rank-reward-payout-service.js";
import { AdminGuildWarehouseGrantService, isAdminGuildWarehouseGrantCommandCandidate } from "./admin-guild-warehouse-grant-service.js";
import { AdminGuildMembershipSyncService, isAdminGuildMembershipSyncCommand } from "./admin-guild-membership-sync-service.js";
import { AdminFullSyncService, isAdminFullSyncCommand } from "./admin-full-sync-service.js";
...
import { AdminDormantAccountDeleteService, isAdminDormantAccountCommand } from "./admin-dormant-account-delete-service.js";
import { AdminDormantAccountRegistryService, isAdminDormantRegistryCommand } from "./admin-dormant-account-registry-service.js";
...
import { AdminGuildResetAllService, isAdminGuildResetAllCommand } from "./admin-guild-reset-all-service.js";
import { isPunchRankResetCommand, PunchRankResetService } from "./punch-rank-reset-service.js";
...
export class IrisAdminCommandService {
  ...
  > {
    ...
    if (isRocketPackageGrantCommandCandidate(input.message)) return this.handleRocketPackageGrant(input);
    if (isSlotNewbiePackageGrantCommandCandidate(input.message)) return this.handleSlotNewbiePackageGrant(input);
    ...
    if (isGuildTerritoryBoosterContributeCandidate(input.message)) return new GuildTerritoryBoosterContributeService(this.database).handleDispatchedIris(input);
    ...
    if (isGuildCreateCandidate(input.message)) return new GuildCreateService(this.database).handleIris(input);
    ...
    if (isGuildTerritoryAttackCommand(input.message)) {
      const attack = await new GuildTerritoryAttackService(this.database).handleIris(input);
      if (attack === null) return { status: "legacy_fallback" };
      if (attack.status === "changed") return { status: "changed", data: attack.result.data, outboxId: attack.result.outboxId };
      return attack;
    }
    if (isGuildRankSnapshotRefreshCommand(input.message)) {
      const rank = await new GuildRankSnapshotRefreshService(this.database).handleIris(input);
      if (rank === null) return { status: "legacy_fallback" };
    ...
    if (isGuildRankRewardPayoutCommand(input.message)) return new GuildRankRewardPayoutService(this.database).handleIris(input);
    if (isOverallRankRewardPayoutCommand(input.message)) return new OverallRankRewardPayoutService(this.database).handleIris(input);
    if (isAdminGuildWarehouseGrantCommandCandidate(input.message)) return new AdminGuildWarehouseGrantService(this.database).handleIris(input);
    if (isAdminGuildMembershipSyncCommand(input.message)) return new AdminGuildMembershipSyncService(this.database).handleIris(input);
    if (isAdminFullSyncCommand(input.message)) return new AdminFullSyncService(this.database).handleIris(input);
    ...
    if (isAdminDormantAccountCommand(input.message)) return new AdminDormantAccountDeleteService(this.database).handleIris(input);
    ...
    if (isAdminGuildResetAllCommand(input.message)) return new AdminGuildResetAllService(this.database).handleIris(input);
    if (isPunchRankResetCommand(input.message)) return new PunchRankResetService(this.database).handleIris(input);
    ...
    if (isMiniPetDirectGrantCommandCandidate(input.message)) return new MiniPetDirectGrantService(this.database).handleIris(input);
  ...
  // 총괄 운영자의 번호별 `/로켓` 지급을 rollout과 단일 권한 뒤 canonical item 경로로 처리합니다.
  async handleRocketPackageGrant(input:{externalUserId:string;channelId:string;message:string;eventId:string}):Promise<{status:"changed";data:string;outboxId:string}|{status:"shadow"|"legacy_fallback"|"handled_no_reply"}>{
    const commandCode="ADMIN_ROCKET_PACKAGE_GRANT",rollout=await this.database.query<Array<{rollout_state:RolloutState;enabled:number}>>("SELECT rollout_state,enabled FROM command_registry WHERE command_code=? LIMIT 1",[commandCode]),definition=rollout[0],dispatch=new MariaCommandDispatchRepository(this.database);
    ...
    const operators=await this.database.query<Array<{operator_id:bigint}>>(`SELECT mapping.operator_id FROM external_identities identity JOIN admin_operator_external_identities mapping ON mapping.external_identity_id=identity.id JOIN admin_operators operator ON operator.id=mapping.operator_id AND operator.status='active' JOIN admin_operator_roles operator_role ON operator_role.operator_id=operator.id JOIN admin_roles role ON role.id=operator_role.role_id AND role.code='super_admin' AND role.active=TRUE JOIN admin_role_permissions permission ON permission.role_id=role.id AND permission.permission_code='inventory.rocket_package.grant' WHERE identity.provider_code='kakao' AND identity.external_user_id=? AND identity.status='linked' LIMIT 1`,[input.externalUserId]);
    if(operators[0]===undefined)return{status:"handled_no_reply"};
    await dispatch.record({eventId:input.eventId,message:input.message,userId:input.externalUserId,hasTrustedDisplayName:true},{route:"MODERN",reasonCode:"MODERN_ROUTE_ALLOWED",commandCode,handlerKey:commandCode});
    const result=await new RocketPackageGrantService(this.database).grant({eventId:input.eventId,destinationId:input.channelId,operatorId:operators[0]!.operator_id.toString(),message:input.message});return{status:"changed",data:result.data,outboxId:result.outboxId};
  }

  // 총괄 운영자의 슬롯초보 패키지 지급을 rollout과 단일 권한 뒤 canonical item 경로로 처리합니다.
  async handleSlotNewbiePackageGrant(input:{externalUserId:string;channelId:string;message:string;eventId:string}):Promise<{status:"changed";data:string;outboxId:string}|{status:"shadow"|"legacy_fallback"|"handled_no_reply"}>{
    const commandCode="ADMIN_SLOT_NEWBIE_PACKAGE_GRANT",rollout=await this.database.query<Array<{rollout_state:RolloutState;enabled:number}>>("SELECT rollout_state,enabled FROM command_registry WHERE command_code=? LIMIT 1",[commandCode]),definition=rollout[0],dispatch=new MariaCommandDispatchRepository(this.database);
    if(definition===undefined||definition.enabled!==1||definition.rollout_state==="LEGACY_ONLY"){await dispatch.record({eventId:input.eventId,message:input.message,userId:input.externalUserId,hasTrustedDisplayName:true},{route:"LEGACY_FALLBACK",reasonCode:"ROLLOUT_LEGACY_ONLY",commandCode,handlerKey:commandCode});return{status:"legacy_fallback"};}
    if(definition.rollout_state==="SHADOW"||definition.rollout_state==="CANARY"){await dispatch.record({eventId:input.eventId,message:input.message,userId:input.externalUserId,hasTrustedDisplayName:true},{route:"SHADOW",reasonCode:"ROLLOUT_SHADOW",commandCode,handlerKey:commandCode});return{status:"shadow"};}
    const operators=await this.database.query<Array<{operator_id:bigint}>>(`SELECT mapping.operator_id FROM external_identities identity JOIN admin_operator_external_identities mapping ON mapping.external_identity_id=identity.id JOIN admin_operators operator ON operator.id=mapping.operator_id AND operator.status='active' JOIN admin_operator_roles operator_role ON operator_role.operator_id=operator.id JOIN admin_roles role ON role.id=operator_role.role_id AND role.code='super_admin' AND role.active=TRUE JOIN admin_role_permissions permission ON permission.role_id=role.id AND permission.permission_code='inventory.slot_newbie_package.grant' WHERE identity.provider_code='kakao' AND identity.external_user_id=? AND identity.status='linked' LIMIT 1`,[input.externalUserId]);
    if(operators[0]===undefined)return{status:"handled_no_reply"};
    await dispatch.record({eventId:input.eventId,message:input.message,userId:input.externalUserId,hasTrustedDisplayName:true},{route:"MODERN",reasonCode:"MODERN_ROUTE_ALLOWED",commandCode,handlerKey:commandCode});
    const result=await new SlotNewbiePackageGrantService(this.database).grant({eventId:input.eventId,destinationId:input.channelId,operatorId:operators[0]!.operator_id.toString(),message:input.message});return{status:"changed",data:result.data,outboxId:result.outboxId};
  }
  ...
  // rollout과 운영자 권한을 확인한 뒤 회원 인증과 관리자 보상을 원자 처리합니다.
  async handleMemberVoiceAuthReward(input: { externalUserId: string; channelId: string; message: string; eventId: string }): Promise<
    { status: "changed"; data: string; outboxId: string } | { status: "shadow" | "legacy_fallback" }
  > {
    const commandCode = "ADMIN_MEMBER_VOICE_AUTH_REWARD";
    const rollout = await this.database.query<Array<{ rollout_state: RolloutState; enabled: number }>>(
      "SELECT rollout_state,enabled FROM command_registry WHERE command_code=? LIMIT 1", [commandCode]
    );
    const definition = rollout[0];
    ...
    const operators = await this.database.query<Array<{ operator_id: bigint; operator_player_id: bigint; display_name: string }>>(
      `SELECT operator.id operator_id,identity.player_id operator_player_id,operator.display_name
       FROM external_identities identity
       JOIN admin_operator_external_identities mapping ON mapping.external_identity_id=identity.id
       JOIN admin_operators operator ON operator.id=mapping.operator_id
       JOIN admin_operator_roles operator_role ON operator_role.operator_id=operator.id
       ...
       JOIN admin_role_permissions permission ON permission.role_id=role.id
       WHERE identity.provider_code='kakao' AND identity.external_user_id=? AND identity.status='linked'
         AND operator.status='active' AND permission.permission_code='player.voice_verification.grant' LIMIT 1`,
      [input.externalUserId]
    );
    const operator = operators[0];
    if (operator === undefined) throw new ApplicationError("FORBIDDEN", "❌ 회원 인증 권한이 없습니다.", 403);
    const result = await new MemberVoiceAuthRewardService(this.database).grant({
      message: input.message,
      idempotencyKey: input.eventId,
      sourceEventId: input.eventId,
      destinationId: input.channelId,
      operatorId: operator.operator_id.toString(),
      operatorPlayerId: operator.operator_player_id.toString(),
