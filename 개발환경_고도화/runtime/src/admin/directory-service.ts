...
export class AdminDirectoryService {
  ...
  async listIdentityCandidates(): Promise<Array<{ id: string; providerCode: string; externalUserId: string; displayName: string | null; playerId: string | null; status: string }>> {
    const rows = await this.database.query<Array<{ id: bigint; provider_code: string; external_user_id: string; display_name: string | null; player_id: bigint | null; status: string }>>(
      `SELECT id, provider_code, external_user_id, display_name, player_id, status
       FROM external_identities WHERE status = 'candidate' ORDER BY id LIMIT 200`
    );
    return rows.map((row) => ({ id: row.id.toString(), providerCode: row.provider_code, externalUserId: row.external_user_id, displayName: row.display_name, playerId: row.player_id?.toString() ?? null, status: row.status }));
  }
  ...
  async listAudit(limit: number): Promise<Array<Record<string, unknown>>> {
    ...
    return rows.map((row) => ({ id: row.id.toString(), operationId: row.operation_id.toString(), actorType: row.actor_type, actorId: row.actor_id?.toString() ?? null, targetType: row.target_type, targetId: row.target_id?.toString() ?? null, actionCode: row.action_code, resultCode: row.result_code, reason: row.reason, createdAt: row.created_at.toISOString() }));
  }
}
