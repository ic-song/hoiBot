...
import { AccountPlatformActiveMemberAuthorityProvider } from "../account-platform/active-member-authority-provider.js";
...
export class PetTitleAdminAppWiringIngress {
  constructor(
    ...
    private readonly activeMembers=new AccountPlatformActiveMemberAuthorityProvider(),
  ){}
  ...
  async sync(input:PetTitleAdminIngressInput):Promise<PetTitleAdminDispatchResult>{
    return this.execute(input,"ADMIN_PET_TITLE_SYNC","admin_pet_title_sync",{},async(database,claim)=>{
      await database.query("SELECT lock_key FROM canonical_account_authority_global_locks WHERE lock_key='ACCOUNT_AUTHORITY' FOR UPDATE");
      await database.query("SELECT lock_key FROM canonical_pet_title_global_locks WHERE lock_key='PET_TITLE' FOR UPDATE");
      const operatorId=await lockOperatorContext(database,input,["super_admin"]);
      const actor=await this.contexts.resolveSelf(database,{identityProviderCode:"kakao",externalUserId:input.externalUserId,externalContextId:input.channelId});
