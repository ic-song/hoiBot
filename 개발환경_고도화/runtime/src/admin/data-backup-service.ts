...
interface DataBackupExecutionContext {
  operatorId: bigint;
...
export class DataBackupService {
  ...
  async handleIris(input: { externalUserId: string; channelId: string; message: string; eventId: string }): Promise<{ status: "changed"; data: string; outboxId: string } | { status: "shadow" | "legacy_fallback" | "handled_no_reply" }> {
    ...
    const result = await this.backup({ eventId: input.eventId, externalUserId: input.externalUserId, destinationId: input.channelId });
  ...
  async backup(input: { eventId: string; externalUserId: string; destinationId: string }): Promise<DataBackupResult | null> {
    ...
    return this.executeBackup({
      ...
      idempotencyKey: normalizeIdempotencyKey(input.eventId),
      reason: "Iris dev/데이터백업",
      sourceCode: "iris",
  ...
  // 인증된 웹 운영자를 기존 PROD→DEV 원자 백업 transaction에 연결합니다.
  async backupForOperator(input: { operatorId: string; idempotencyKey: string; reason: string }): Promise<DataBackupResult> {
    return this.executeBackup({
  ...
  private async executeBackup(context: DataBackupExecutionContext): Promise<DataBackupResult> {
    return withDeadlockRetry(() => this.database.withTransaction(async (transaction) => {
      const previous = (await transaction.query<Array<{ result_json: string | DataBackupResult | null }>>("SELECT result_json FROM operations WHERE idempotency_scope='data_backup.execute' AND idempotency_key=? FOR UPDATE", [context.idempotencyKey]))[0];
      ...
      for (const object of source) {
        if (!isTopLevelFileName(object.file_name)) throw new Error(`Nested source object blocked: ${object.file_name}`);
        const hash = createHash("sha256").update(object.payload_text).digest("hex");
        const size = BigInt(Buffer.byteLength(object.payload_text, "utf8"));
        if (hash !== object.content_sha256 || size !== BigInt(object.size_bytes)) throw new Error(`Source object integrity mismatch: ${object.file_name}`);
      }
      await transaction.query("SELECT id FROM managed_data_objects WHERE environment_code='dev' ORDER BY file_name ASC FOR UPDATE");
      const sourceRevisionKey = buildSourceRevisionKey(source);
      const operation = await transaction.execute("INSERT INTO operations(operation_key,idempotency_scope,idempotency_key,actor_type,actor_id,source_code,status,created_at) VALUES (?,'data_backup.execute',?,'admin_operator',?,?,'processing',UTC_TIMESTAMP(3))", [randomUUID(), context.idempotencyKey, context.operatorId, context.sourceCode]);
      ...
      for (const object of source) {
        const before = (await transaction.query<Array<{ content_sha256: string }>>("SELECT content_sha256 FROM managed_data_objects WHERE environment_code='dev' AND file_name=? LIMIT 1", [object.file_name]))[0];
        await transaction.execute("INSERT INTO backup_manifest(run_id,file_name,source_object_id,source_revision,content_sha256,size_bytes,modified_at) VALUES (?,?,?,?,?,?,?)", [run.insertId, object.file_name, object.id, object.revision_version, object.content_sha256, object.size_bytes, object.modified_at]);
        const target = await transaction.execute("INSERT INTO managed_data_objects(environment_code,file_name,payload_text,content_sha256,size_bytes,revision_version,modified_at) VALUES ('dev',?,?,?,?,1,UTC_TIMESTAMP(3)) ON DUPLICATE KEY UPDATE id=LAST_INSERT_ID(id),payload_text=VALUES(payload_text),content_sha256=VALUES(content_sha256),size_bytes=VALUES(size_bytes),revision_version=revision_version+1,modified_at=UTC_TIMESTAMP(3)", [object.file_name, object.payload_text, object.content_sha256, object.size_bytes]);
        await transaction.execute("INSERT INTO backup_run_items(run_id,file_name,target_object_id,item_status,before_sha256,after_sha256,completed_at) VALUES (?,?,?,'copied',?,?,UTC_TIMESTAMP(3))", [run.insertId, object.file_name, target.insertId, before?.content_sha256 ?? null, object.content_sha256]);
        copiedFiles.push(object.file_name);
      }
      await transaction.execute("UPDATE backup_runs SET run_status='complete',completed_at=UTC_TIMESTAMP(3) WHERE id=?", [run.insertId]);
      const data = formatDataBackupResult(copiedFiles);
      let outboxId: string | null = null;
      if (context.destinationId !== undefined && context.eventId !== undefined) {
        const outbox = await transaction.execute("INSERT INTO outbox_messages(operation_id,provider_code,destination_id,message_type,payload_json,status,available_at,created_at) VALUES (?,'iris',?,'text',?,'pending',UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))", [operation.insertId, context.destinationId, JSON.stringify({ data })]);
        outboxId = outbox.insertId.toString();
        await transaction.execute("INSERT INTO command_executions(event_id,command_code,operation_id,execution_status,result_code,created_at,completed_at) VALUES (?,'ADMIN_DATA_BACKUP',?,'completed','reply_queued',UTC_TIMESTAMP(3),UTC_TIMESTAMP(3))", [context.eventId, operation.insertId]);
      }
      await transaction.execute("INSERT INTO command_audit(operation_id,actor_type,actor_id,target_type,target_id,action_code,result_code,reason,change_summary_json,created_at) VALUES (?,'admin_operator',?,'backup_run',?,'data_backup.execute','success',?,?,UTC_TIMESTAMP(3))", [operation.insertId, context.operatorId, run.insertId, context.reason, JSON.stringify({ sourceEnvironment: "prod", targetEnvironment: "dev", sourceRevisionKey, copiedCount: copiedFiles.length, copiedFiles })]);
      const result: DataBackupResult = { runId: run.insertId.toString(), sourceRevisionKey, copiedFiles, copiedCount: copiedFiles.length, data, outboxId };
