...
export const ADMIN_WEB_CLIENT = String.raw`(function () {
  ...
  // canonical 정의 규모와 package correction 결과를 하나의 관리 projection으로 표시합니다.
  function assetCatalogOverview(catalog) {
    var domains = "<div class=\"metric-grid compact\">" + catalog.domains.map(function (domain) { return "<article class=\"metric-card\"><span>" + escapeHtml(domain.domain) + "</span><strong>" + formatNumber(domain.count) + "</strong><small class=\"mono\">" + escapeHtml(domain.canonicalTable) + "</small></article>"; }).join("") + "</div>";
    var value = catalog.packageResolution; if (!value) return domains;
    var gaps = value.gaps.map(function (gap) { return "<tr><td class=\"mono\">" + escapeHtml(gap.sourceOrder) + "</td><td>" + escapeHtml(gap.targetType) + "</td><td>" + escapeHtml(gap.displayName) + "</td><td><span class=\"catalog-status inactive\">GAP</span></td></tr>"; }).join("");
    return domains + "<section class=\"panel\"><div class=\"panel-heading\"><div><h3>패키지 canonical 해소 현황</h3><p>동결값을 보존하고 correction overlay만 유효값으로 합산합니다.</p></div><div class=\"catalog-summary\"><span>" + escapeHtml(value.publicationStatus) + "</span><span class=\"mono\">" + escapeHtml(value.catalogVersion) + "</span></div></div><div class=\"metric-grid compact\"><article class=\"metric-card\"><span>유효 해소</span><strong>" + formatNumber(value.effectiveResolved) + "</strong><small>frozen " + formatNumber(value.frozenResolved) + " + overlay " + formatNumber(value.overlayResolved) + "</small></article><article class=\"metric-card\"><span>STACK GAP</span><strong>" + formatNumber(value.residualStackGaps) + "</strong></article><article class=\"metric-card\"><span>PACKAGE GAP</span><strong>" + formatNumber(value.residualPackageGaps) + "</strong></article><article class=\"metric-card\"><span>충돌</span><strong>" + formatNumber(value.conflicts) + "</strong></article></div><div class=\"table-wrap\"><table class=\"catalog-table\"><thead><tr><th>순서</th><th>유형</th><th>표시명</th><th>상태</th></tr></thead><tbody>" + (gaps || "<tr><td colspan=\"4\">잔여 GAP이 없습니다.</td></tr>") + "</tbody></table></div></section>";
  }

  // stable key와 source binding을 중심으로 검색 결과를 표시합니다.
  function assetCatalogTable(catalog) {
    var rows = catalog.items.map(function (object) { var source = object.sourceBindings.map(function (binding) { return binding.system + ":" + binding.table; }).slice(0, 2).join(" · ") || "연결 없음"; return "<tr><td><button class=\"table-link mono\" type=\"button\" data-object-key=\"" + escapeHtml(object.objectKey) + "\">" + escapeHtml(object.objectKey) + "</button><small>" + escapeHtml(source) + "</small></td><td>" + escapeHtml(object.objectType) + "</td><td>" + escapeHtml(object.displayName) + "</td><td><span class=\"catalog-status " + (object.active ? "" : "inactive") + "\">" + (object.active ? "활성" : "비활성") + "</span></td><td class=\"mono\">v" + escapeHtml(object.version) + "</td></tr>"; }).join("");
    return "<div class=\"table-wrap\"><table class=\"catalog-table\"><thead><tr><th>Stable key / source</th><th>유형</th><th>표시명</th><th>가용성</th><th>버전</th></tr></thead><tbody>" + (rows || "<tr><td colspan=\"5\">조건에 맞는 자산이 없습니다.</td></tr>") + "</tbody></table></div>";
  }

  // manager/super_admin에게 통합 목록과 기존 REGISTER·UPDATE·SET_ACTIVE를 제공합니다.
  async function loadObjectCatalog(objectKey) {
    ...
    var main = byId("main-content");
    var params = new URLSearchParams({ page: "1", limit: "25" }); if (state.objectCatalogQuery) params.set("query", state.objectCatalogQuery); if (state.objectCatalogType) params.set("objectType", state.objectCatalogType); if (state.objectCatalogActive) params.set("active", state.objectCatalogActive);
    main.innerHTML = renderViewIntro("자산 카탈로그", "canonical 정의와 패키지 GAP을 불러오는 중입니다.", "manager / super_admin") + loadingState("자산 카탈로그를 불러오는 중");
    var catalog; try { catalog = (await api("/api/v1/admin/object-catalog/objects?" + params.toString())).catalog; } catch (error) { main.innerHTML = renderViewIntro("자산 카탈로그", "canonical 자산 조회를 완료하지 못했습니다.") + errorState(error, "object-catalog"); attachRetry(main); return; }
    var options = ["ITEM","PET","FURNITURE","TITLE","PET_TITLE","PACKAGE","CURRENCY","SKILL","HOME_BUILDING","MINI_PET","BADGE"].map(function (type) { return "<option value=\"" + type + "\" " + (state.objectCatalogType === type ? "selected" : "") + ">" + type + "</option>"; }).join("");
    var lookup = "<section class=\"panel\"><div class=\"panel-heading\"><div><h3>통합 자산 목록</h3><p>표시명 또는 stable key로 검색합니다.</p></div><span>" + formatNumber(catalog.total) + "건</span></div><form id=\"object-filter-form\" class=\"object-lookup-form\"><input name=\"query\" value=\"" + escapeHtml(state.objectCatalogQuery) + "\" placeholder=\"stable key 또는 표시명\"><select name=\"objectType\"><option value=\"\">전체 유형</option>" + options + "</select><select name=\"active\"><option value=\"\">전체 상태</option><option value=\"true\" " + (state.objectCatalogActive === "true" ? "selected" : "") + ">활성</option><option value=\"false\" " + (state.objectCatalogActive === "false" ? "selected" : "") + ">비활성</option></select><button class=\"primary-button\" type=\"submit\">검색</button></form>" + assetCatalogTable(catalog) + "<div id=\"object-detail\">" + (state.objectCatalogKey ? loadingState("오브젝트를 불러오는 중") : emptyState("목록에서 자산을 선택하세요.", "stable key·source·version을 확인한 뒤 변경할 수 있습니다.")) + "</div></section>";
    ...
    main.innerHTML = renderViewIntro("자산 카탈로그", "도메인별 canonical 정의와 패키지 GAP을 한 화면에서 조회하고 기존 provider로만 변경합니다.", "manager / super_admin") + objectCatalogNotice() + assetCatalogOverview(catalog) + "<div class=\"catalog-layout\">" + lookup + register + "</div>";
    byId("object-filter-form").addEventListener("submit", function (event) { event.preventDefault(); var data = new FormData(event.currentTarget); state.objectCatalogQuery = data.get("query").toString().trim(); state.objectCatalogType = data.get("objectType").toString(); state.objectCatalogActive = data.get("active").toString(); loadObjectCatalog(state.objectCatalogKey); });
    main.querySelectorAll("[data-object-key]").forEach(function (button) { button.addEventListener("click", function () { loadObjectCatalog(button.dataset.objectKey); }); });
