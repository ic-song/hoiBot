...
export class GuildTerritoryWarFinishService{
  ...
  private async grantResource(tx:DatabaseTransaction,operationId:bigint,sequence:number,guildId:string,currencyCode:string,delta:string){await tx.execute("INSERT IGNORE INTO guild_resource_accounts(guild_id,currency_code,balance,version) VALUES (?,?,0,0)",[guildId,currencyCode]);const account=(await tx.query<Array<{balance:string;version:bigint}>>("SELECT CAST(balance AS CHAR) balance,version FROM guild_resource_accounts WHERE guild_id=? AND currency_code=? FOR UPDATE",[guildId,currencyCode]))[0]!,after=(BigInt(account.balance.split('.')[0]??"0")+BigInt(delta)).toString();await tx.execute("UPDATE guild_resource_accounts SET balance=?,version=version+1 WHERE guild_id=? AND currency_code=? AND version=?",[after,guildId,currencyCode,account.version]);await tx.execute("INSERT INTO guild_resource_ledger(operation_id,sequence_no,guild_id,currency_code,delta,balance_after,reason_code) VALUES (?,?,?,?,?,?, 'guild_territory_finish_reward')",[operationId,sequence,guildId,currencyCode,delta,after]);}
