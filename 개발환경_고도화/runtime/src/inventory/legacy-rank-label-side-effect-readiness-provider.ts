...
interface ReadinessRow {
  ...
  common_staging_run_id:string;
  member_source_path_sha256:string;
  member_source_content_sha256:string;
  guild_source_path_sha256:string;
...
export class LegacyRankLabelSideEffectReadinessProvider {
  async resolve(database:AppWiringReadParticipant,context:PlayerContext):Promise<LegacyRankLabelSideEffectReadiness>{
    try{
      const rows=await database.query<ReadinessRow[]>(`SELECT validation.legacy_rank_label_validation_run_id,validation.object_domain_import_run_id,validation.raw_landing_run_id,validation.common_staging_run_id,
          validation.member_source_path_sha256,validation.member_source_content_sha256,validation.guild_source_path_sha256,validation.guild_source_content_sha256,validation.rank_marker_source_fingerprint,validation.membership_semantic_sha256,validation.projection_version,validation.run_status,
          ...
          ,(SELECT GROUP_CONCAT(CONCAT_WS('|',aggregate_certificate.source_player_key_sha256,COALESCE(aggregate_certificate.player_id,''),aggregate_certificate.member_presence_state,aggregate_certificate.guild_pointer_state,aggregate_certificate.side_effect_decision,aggregate_certificate.guild_pointer_fingerprint,aggregate_certificate.membership_evidence_fingerprint,aggregate_certificate.certificate_fingerprint,
              (SELECT COUNT(DISTINCT binding_receipt.target_pk_value) FROM data_migration_object_domain_import_records binding_receipt JOIN data_migration_catalog_projection_records binding_projection ON binding_projection.catalog_projection_record_id=binding_receipt.catalog_projection_record_id JOIN data_migration_catalog_source_decisions binding_decision ON binding_decision.catalog_source_decision_id=binding_projection.catalog_source_decision_id JOIN data_migration_common_staging_records binding_staging ON binding_staging.common_staging_record_id=binding_decision.common_staging_record_id WHERE binding_receipt.object_domain_import_run_id=validation.object_domain_import_run_id AND binding_projection.catalog_projection_run_id=import_run.catalog_projection_run_id AND binding_decision.catalog_projection_run_id=binding_projection.catalog_projection_run_id AND binding_staging.common_staging_run_id=validation.common_staging_run_id AND binding_staging.source_path_sha256=validation.member_source_path_sha256 AND binding_receipt.target_table_name='canonical_players' AND binding_receipt.target_pk_column_name='player_id' AND binding_staging.owner_locator_sha256=aggregate_certificate.source_player_key_sha256),
              (SELECT COUNT(*) FROM data_migration_object_domain_import_records binding_receipt JOIN data_migration_catalog_projection_records binding_projection ON binding_projection.catalog_projection_record_id=binding_receipt.catalog_projection_record_id JOIN data_migration_catalog_source_decisions binding_decision ON binding_decision.catalog_source_decision_id=binding_projection.catalog_source_decision_id JOIN data_migration_common_staging_records binding_staging ON binding_staging.common_staging_record_id=binding_decision.common_staging_record_id WHERE binding_receipt.object_domain_import_run_id=validation.object_domain_import_run_id AND binding_projection.catalog_projection_run_id=import_run.catalog_projection_run_id AND binding_decision.catalog_projection_run_id=binding_projection.catalog_projection_run_id AND binding_staging.common_staging_run_id=validation.common_staging_run_id AND binding_staging.source_path_sha256=validation.member_source_path_sha256 AND binding_receipt.target_table_name='canonical_players' AND binding_receipt.target_pk_column_name='player_id' AND binding_receipt.target_pk_value=aggregate_certificate.player_id AND binding_staging.owner_locator_sha256=aggregate_certificate.source_player_key_sha256)) ORDER BY aggregate_certificate.source_player_key_sha256 SEPARATOR '\n') FROM legacy_rank_label_side_effect_certificates aggregate_certificate WHERE aggregate_certificate.legacy_rank_label_validation_run_id=validation.legacy_rank_label_validation_run_id) all_certificate_rows
        ...
        JOIN data_migration_catalog_projection_runs catalog ON catalog.catalog_projection_run_id=import_run.catalog_projection_run_id AND catalog.common_staging_run_id=validation.common_staging_run_id AND catalog.run_status='COMPLETE'
        JOIN data_migration_common_staging_runs staging ON staging.common_staging_run_id=validation.common_staging_run_id AND staging.common_staging_run_id=catalog.common_staging_run_id AND staging.run_status='COMPLETE' AND staging.raw_bundle_sha256=raw_run.bundle_sha256 AND staging.snapshot_manifest_sha256=raw_run.snapshot_manifest_sha256
      ...
      const calculatedValidation=calculateLegacyRankLabelValidationFingerprint({rawLandingRunId:row.raw_landing_run_id,commonStagingRunId:row.common_staging_run_id,objectDomainImportRunId:row.object_domain_import_run_id,memberSourcePathSha256:row.member_source_path_sha256,memberSourceContentSha256:row.member_source_content_sha256,guildSourcePathSha256:row.guild_source_path_sha256,guildSourceContentSha256:row.guild_source_content_sha256,rankMarkerSourceFingerprint:row.rank_marker_source_fingerprint,legacyRuntimeSourceSha256:row.legacy_runtime_source_sha256,membershipSemanticSha256:row.membership_semantic_sha256,certificateSetSha256:row.certificate_set_sha256,counts:actualCounts,status:row.run_status,projectionVersion:row.projection_version});
