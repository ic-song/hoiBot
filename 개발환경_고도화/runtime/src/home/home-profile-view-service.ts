...
export class HomeProfileViewService {
  ...
  async execute(input: { eventId: string; externalUserId: string; destinationId: string; message: string }): Promise<Result> {
    ...
    return this.database.withTransaction(async (tx) => {
      ...
      const feeds = ownerPass ? await tx.query<Array<{ content: string }>>("SELECT content FROM home_feeds WHERE home_player_id=? AND deleted_at IS NULL ORDER BY display_order ASC,id ASC LIMIT 10", [home.player_id]) : [];
