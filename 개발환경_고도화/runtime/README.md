...
신규 사이트 가입 흐름은 로그인 ID, 비밀번호, 정확한 `한글 2글자 + 공백 + 남/여` 시스템 이름과 이용약관 동의를 받은 뒤 아직 player를 만들지 않은 미인증 계정을 생성합니다. 개인정보 처리방침의 별도 동의는 받지 않고 최소 계정정보 처리 안내만 표시합니다. 가입 API는 30분 유효 코드와 함께 그대로 복사할 수 있는 `/가입인증 ABCD2345` 명령을 반환합니다. 사용자가 현재 KakaoTalk 닉네임을 시스템 이름과 동일하게 맞추고 이 명령을 입력하면 provider key·닉네임·코드를 함께 검증해 player와 초기 데이터를 한 트랜잭션으로 생성합니다. 닉네임이 다르면 Iris가 현재 전달한 이름과 필요한 시스템 이름을 함께 안내합니다. 가입용 `/가입인증`과 기존 계정 연결용 `/계정인증`은 서로 다른 challenge 타입으로 관리하며, 이번 흐름에서는 가입용 명령만 처리합니다. CAPTCHA는 이번 1차 구현에 포함하지 않습니다.
...
npm.cmd run db:probe:random-box-craft # hoibot_schema_design 격리 DB에서만 실행 가능
npm.cmd run db:probe:furniture-boutique-box-craft # hoibot_schema_design 격리 DB에서만 실행 가능
