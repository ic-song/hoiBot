...
-- 펫먹이던전박스 직접 실행 이력을 stable key 기반 범위 보상 catalog seed로 교정합니다.
INSERT INTO package_item_definitions
  (item_id,item_type,item_name,stackable,metadata_json,enabled,row_version)
VALUES
  ('pet_food_dungeon_box','STACK','펫먹이던전박스🍼(/펫먹이박스오픈)',1,
   JSON_OBJECT('source','legacy-main.js','objectKey','pet_food_dungeon_box',
               'sourceCommand','/펫먹이박스오픈','useRoute','/패키지사용',
               'migrationPolicy','CATALOG_SEED_ONLY'),0,1),
  ('pet_food','STACK','펫먹이🍼',1,
   JSON_OBJECT('source','legacy-main.js','objectKey','pet_food',
...
WHERE item_id IN ('pet_food_dungeon_box','pet_food')
ON DUPLICATE KEY UPDATE
...
VALUES
  ('PKG-PET-FOOD-DUNGEON-BOX','DRAFT-20260826','펫먹이던전박스🍼',
   '개봉마다 펫먹이 40~50개 균등 범위 보상',
   'pet_food_dungeon_box','/펫먹이박스오픈',42,10000,0,'EXACT_LEGACY_DRAFT',0,1)
ON DUPLICATE KEY UPDATE
...
VALUES
  ('RULE-PKG-PET-FOOD-DUNGEON-001','PKG-PET-FOOD-DUNGEON-BOX',1,'PET_FOOD_RANGE',
   'UNIFORM_RANGE','ADD','USER','pet_food',1,NULL,40,50,1,NULL,
   JSON_OBJECT('sourceCommand','/펫먹이박스오픈','objectKey','pet_food',
...
WHERE package_id='PKG-PET-FOOD-DUNGEON-BOX';

DELETE FROM command_aliases
WHERE command_text='/펫먹이박스오픈';
