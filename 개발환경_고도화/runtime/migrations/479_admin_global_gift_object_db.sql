...
CREATE TABLE canonical_admin_global_gift_operations (
  ...
  CONSTRAINT chk_odbt_479_02_hash CHECK (request_identity_fingerprint REGEXP '^[0-9a-f]{64}$' AND payload_fingerprint REGEXP '^[0-9a-f]{64}$' AND request_fingerprint REGEXP '^[0-9a-f]{64}$' AND announcement_fingerprint REGEXP '^[0-9a-f]{64}$' AND (result_fingerprint IS NULL OR result_fingerprint REGEXP '^[0-9a-f]{64}$')),
  CONSTRAINT chk_odbt_479_02_state CHECK ((operation_status='PROCESSING' AND result_fingerprint IS NULL AND terminal_json IS NULL) OR (operation_status='STAGED' AND result_fingerprint IS NOT NULL AND terminal_json IS NULL) OR (operation_status='COMPLETED' AND result_fingerprint IS NOT NULL AND JSON_VALID(terminal_json))),
