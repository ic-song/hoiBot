-- WBS734 depends on 444_canonical_item_inventory.sql, which creates canonical_players first.
-- Legacy bigint/id/code furniture tables are retained for compatibility.
...
CREATE TABLE IF NOT EXISTS object_owned_furniture_instances (
  ...
  CONSTRAINT fk_object_owned_furniture_player FOREIGN KEY (player_id) REFERENCES canonical_players (player_id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS object_furniture_operation_replays (
  ...
  CONSTRAINT fk_object_furniture_operation_player FOREIGN KEY (player_id) REFERENCES canonical_players (player_id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS object_furniture_ownership_history (
  furniture_ownership_history_id CHAR(8) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  owned_furniture_id CHAR(8) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  furniture_operation_id CHAR(8) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  status_before VARCHAR(20) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  status_after VARCHAR(20) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  INSERT_USER VARCHAR(100) NOT NULL, INSERT_TIME CHAR(19) NOT NULL, UPDATE_USER VARCHAR(100) NOT NULL, UPDATE_TIME CHAR(19) NOT NULL,
  PRIMARY KEY (furniture_ownership_history_id),
  CONSTRAINT fk_object_furniture_history_owned FOREIGN KEY (owned_furniture_id) REFERENCES object_owned_furniture_instances (owned_furniture_id) ON DELETE RESTRICT,
  CONSTRAINT fk_object_furniture_history_operation FOREIGN KEY (furniture_operation_id) REFERENCES object_furniture_operation_replays (furniture_operation_id) ON DELETE RESTRICT,
  CONSTRAINT chk_object_furniture_history_status CHECK (status_before IN ('bag','placed','listed','sold','removed') AND status_after IN ('bag','placed','listed','sold','removed')),
  CONSTRAINT chk_object_furniture_history_insert_time CHECK (INSERT_TIME REGEXP '^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01]) ([01][0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$'),
  CONSTRAINT chk_object_furniture_history_update_time CHECK (UPDATE_TIME REGEXP '^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01]) ([01][0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$')
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS object_furniture_market_listings (
  furniture_market_listing_id CHAR(8) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  owned_furniture_id CHAR(8) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  listing_price BIGINT UNSIGNED NOT NULL,
  listing_status VARCHAR(20) CHARACTER SET ascii COLLATE ascii_bin NOT NULL DEFAULT 'active',
  INSERT_USER VARCHAR(100) NOT NULL, INSERT_TIME CHAR(19) NOT NULL, UPDATE_USER VARCHAR(100) NOT NULL, UPDATE_TIME CHAR(19) NOT NULL,
  PRIMARY KEY (furniture_market_listing_id), UNIQUE KEY uq_object_furniture_market_listing_owned (owned_furniture_id),
  CONSTRAINT fk_object_furniture_market_owned FOREIGN KEY (owned_furniture_id) REFERENCES object_owned_furniture_instances (owned_furniture_id) ON DELETE RESTRICT,
  CONSTRAINT chk_object_furniture_market_status CHECK (listing_status IN ('active','cancelled','sold')),
  CONSTRAINT chk_object_furniture_market_insert_time CHECK (INSERT_TIME REGEXP '^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01]) ([01][0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$'),
  CONSTRAINT chk_object_furniture_market_update_time CHECK (UPDATE_TIME REGEXP '^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01]) ([01][0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$')
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
