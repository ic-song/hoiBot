...
-- Existing ADMIN_SYNC receipts may only be upgraded when an unambiguous display value can be recovered.
-- A synthetic player_id/source_identifier fallback would turn an internal identifier into a user-visible nickname.
SET @migration_474_preflight = IF(EXISTS (
      SELECT 1
      ...
      WHERE batch_row.operation_type='ADMIN_SYNC'
        AND COALESCE(CHAR_LENGTH(profile.current_display_name) BETWEEN 1 AND 255,FALSE)=FALSE
        AND (SELECT COUNT(DISTINCT BINARY TRIM(identity_row.display_name))
              ...
              WHERE crosswalk.player_id=target_row.player_id AND crosswalk.crosswalk_status='LINKED') > 1
    ),
    'SIGNAL SQLSTATE ''45000'' SET MESSAGE_TEXT=''PET_TITLE_BATCH_MEMBER_KEY_AMBIGUOUS''',
    'DO 0');
PREPARE migration_474_preflight_statement FROM @migration_474_preflight;
EXECUTE migration_474_preflight_statement;
DEALLOCATE PREPARE migration_474_preflight_statement;
      ...
      WHERE batch_row.operation_type='ADMIN_SYNC'
        ...
        AND (SELECT COUNT(DISTINCT BINARY TRIM(identity_row.display_name))
               ...
               JOIN external_identities identity_row
                ...
                AND identity_row.status='linked'
                AND CHAR_LENGTH(TRIM(identity_row.display_name)) BETWEEN 1 AND 255
              WHERE crosswalk.player_id=target_row.player_id AND crosswalk.crosswalk_status='LINKED') = 0
    ),
    'SIGNAL SQLSTATE ''45000'' SET MESSAGE_TEXT=''PET_TITLE_BATCH_MEMBER_KEY_UNRESOLVED''',
    'DO 0');
PREPARE migration_474_preflight_statement FROM @migration_474_preflight;
EXECUTE migration_474_preflight_statement;
DEALLOCATE PREPARE migration_474_preflight_statement;
...
SET target_row.member_key_before=COALESCE(
  ...
  (SELECT CASE
            WHEN COUNT(DISTINCT BINARY TRIM(identity_row.display_name))=1
            THEN MIN(TRIM(identity_row.display_name))
          END
     FROM canonical_player_identity_crosswalks crosswalk
     JOIN external_identities identity_row
       ON identity_row.provider_code=crosswalk.provider_code
...
WHERE target_row.member_key_before IS NULL
  AND batch_row.operation_type='ADMIN_SYNC'
  AND (
    CHAR_LENGTH(profile.current_display_name) BETWEEN 1 AND 255
    OR (SELECT COUNT(DISTINCT BINARY TRIM(identity_row.display_name))
          FROM canonical_player_identity_crosswalks crosswalk
          JOIN external_identities identity_row
            ON identity_row.provider_code=crosswalk.provider_code
           AND identity_row.external_user_id=crosswalk.external_user_id
