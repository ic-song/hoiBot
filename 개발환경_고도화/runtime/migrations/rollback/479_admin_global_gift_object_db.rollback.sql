SET @rollback_479_external_reference_predicate=(SELECT GROUP_CONCAT(CONCAT('EXISTS(SELECT 1 FROM `',REPLACE(TABLE_NAME,'`','``'),'` WHERE `',REPLACE(COLUMN_NAME,'`','``'),'`=''j7uyw6vc'' LIMIT 1)') SEPARATOR ' OR ') FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_SCHEMA=DATABASE() AND REFERENCED_TABLE_NAME='canonical_item_definitions' AND REFERENCED_COLUMN_NAME='item_id' AND TABLE_NAME NOT IN ('canonical_item_definition_imports','canonical_admin_global_gift_operations'));
SET @rollback_479_external_reference_sql=CONCAT('SELECT ',COALESCE(@rollback_479_external_reference_predicate,'FALSE'),' INTO @rollback_479_external_reference_exists');
PREPARE rollback_479_reference_statement FROM @rollback_479_external_reference_sql;
EXECUTE rollback_479_reference_statement;
DEALLOCATE PREPARE rollback_479_reference_statement;
SET @rollback_479_extra_import_exists=EXISTS(SELECT 1 FROM canonical_item_definition_imports WHERE item_id='j7uyw6vc' AND item_definition_import_id<>'phk8c656' LIMIT 1);
SET @rollback_479_guard=IF(EXISTS(SELECT 1 FROM canonical_admin_global_gift_operations LIMIT 1) OR (EXISTS(SELECT 1 FROM canonical_admin_global_gift_migration_provenance WHERE admin_global_gift_migration_provenance_id='n89yb2ye' AND catalog_seed_created_flag=TRUE) AND (@rollback_479_extra_import_exists OR @rollback_479_external_reference_exists)),'SIGNAL SQLSTATE ''45000'' SET MESSAGE_TEXT=''ADMIN_GLOBAL_GIFT_DURABLE_STATE_EXISTS''','DO 0');
