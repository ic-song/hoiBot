-- Destructive rollback: only before EVENT_CONTROL MODERN cutover.
-- Fail before DDL while a durable EVENT_CONTROL receipt still depends on this typed column/table.
SELECT (
  ...
  FROM (
    ...
    WHERE EXISTS (
      ...
      WHERE receipt_kind = 'PET_EXPLORE_EVENT_CONTROL'
    )
...
ALTER TABLE IF EXISTS canonical_app_wiring_receipt_links
  ...
  DROP FOREIGN KEY IF EXISTS fk_odbt_470_02_01,
  DROP INDEX IF EXISTS uq_odbt_470_02_01,
  DROP COLUMN IF EXISTS pet_explore_event_control_operation_id,
  ADD CONSTRAINT IF NOT EXISTS chk_odbt_466_02_rule_02 CHECK ((daily_prayer_operation_id IS NOT NULL) + (home_aggregate_operation_id IS NOT NULL) + (market_operation_id IS NOT NULL) + (member_title_operation_id IS NOT NULL) + (mini_pet_title_operation_id IS NOT NULL) + (package_use_operation_id IS NOT NULL) + (pet_explore_operation_id IS NOT NULL) + (pet_title_operation_id IS NOT NULL) + (player_identity_operation_id IS NOT NULL) = 1),
  ADD CONSTRAINT IF NOT EXISTS chk_odbt_466_02_rule_03 CHECK ((receipt_kind = 'DAILY_PRAYER' AND daily_prayer_operation_id IS NOT NULL) OR (receipt_kind = 'HOME_AGGREGATE' AND home_aggregate_operation_id IS NOT NULL) OR (receipt_kind = 'MARKET' AND market_operation_id IS NOT NULL) OR (receipt_kind = 'MEMBER_TITLE' AND member_title_operation_id IS NOT NULL) OR (receipt_kind = 'MINI_PET_TITLE' AND mini_pet_title_operation_id IS NOT NULL) OR (receipt_kind = 'PACKAGE_USE' AND package_use_operation_id IS NOT NULL) OR (receipt_kind = 'PET_EXPLORE' AND pet_explore_operation_id IS NOT NULL) OR (receipt_kind = 'PET_TITLE' AND pet_title_operation_id IS NOT NULL) OR (receipt_kind = 'PLAYER_IDENTITY' AND player_identity_operation_id IS NOT NULL));

DROP TABLE IF EXISTS canonical_pet_explore_event_control_operations;
