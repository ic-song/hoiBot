...
-- 정령 박스는 독립 실행 명령이 아니라 /패키지사용으로 여는 카탈로그 seed로만 등록합니다.
INSERT INTO package_item_definitions
  (item_id,item_type,item_name,stackable,metadata_json,enabled,row_version)
VALUES
  ('ITEM-DUNGEON-SPIRIT-BOX','STACK','정령박스🥀(/정령박스오픈)',1,
   JSON_OBJECT('source','legacy-main.js','objectKey','spirit_dungeon_box',
               'sourceCommand','/정령박스오픈','useRoute','/패키지사용',
...
WHERE item_id='ITEM-DUNGEON-SPIRIT-BOX'
ON DUPLICATE KEY UPDATE
...
VALUES
  ('PKG-SPIRIT-DUNGEON-BOX','DRAFT-20260826','정령박스🥀',
   '개봉당 정령 강화석 100개 고정 보상',
   'ITEM-DUNGEON-SPIRIT-BOX','/정령박스오픈',50,10000,0,'EXACT_LEGACY_DRAFT',0,1)
ON DUPLICATE KEY UPDATE
...
VALUES
  ('RULE-PKG-SPIRIT-DUNGEON-001','PKG-SPIRIT-DUNGEON-BOX',1,'ALL','ALL','ADD','USER',
   'ITEM-RWD-023',100,NULL,NULL,NULL,NULL,NULL,
   JSON_OBJECT('sourceCommand','/정령박스오픈','objectKey','spirit_stone',
               'rollPolicy','FIXED_PER_OPEN'),NULL,1)
ON DUPLICATE KEY UPDATE
...
WHERE package_id='PKG-SPIRIT-DUNGEON-BOX';

DELETE FROM command_aliases
WHERE command_text='/정령박스오픈';
