...
try {
  ...
  $successMessage="PET_TITLE_SALE_MARIADB_PASS port=$rehearsalPort database=$rehearsalDatabase initialPid=$initialPid restartPid=$restartPid migration471Forward=true reentry=true productionIngress=true productionDispatcher=true playerContext=true shadowLegacyFallback=true shadowDomainDmlZero=true selectionWarLockOrder=true selectionWarLockCompetition=true readySaleAtomic=true pendingStartSale=true activeOpeningNoReply=true outboxZero=true replayNoDoubleCredit=true silentImmediateReplayDmlZero=true restartReplayDmlZero=true silentRestartReplayDmlZero=true sameTypedReceiptLink=true outboxFaultRollback=true liveReceiptRollbackPreflight=true sellReceiptsRemovedRollback=true rollback=true reforward=true production3306Unchanged=true"
} finally {
