...
const priorPath = "개발환경_고도화/migration-control/fixtures/synthetic-relational/object-db-consumer-execution-receipts-wave22-v1.json";
const fixturePath = "개발환경_고도화/migration-control/contracts/object-db-consumer-executable-parity-wave23-mutations-v1.json";
const harnessPath = "개발환경_고도화/runtime/test/fixtures/object-db-executable-parity-wave23-mutations-harness.mjs";
const targetPath = "개발환경_고도화/runtime/test/fixtures/object-db-executable-parity-wave23-target.mjs";
const outputPath = "개발환경_고도화/migration-control/fixtures/synthetic-relational/object-db-consumer-execution-receipts-wave23-v1.json";
const evidenceCommit = process.argv[2];
if (typeof evidenceCommit !== "string" || !/^[0-9a-f]{40}$/.test(evidenceCommit)) throw new Error("Wave23 exact evidenceCommit required");
const committed = (path: string): string => {
  ...
  if (!match?.[1]) throw new Error(`Wave23 committed blob missing: ${path}`);
...
if (prior.receipts.length !== 225 || Buffer.byteLength(prefix, "utf8") !== 919668 || sha256CanonicalText(prefix) !== "1a7bec9fefe0c009012fbaa3286f987a1ae8b8d279cbd23ab0870e4919d83809") throw new Error("Wave23 immutable Wave22 receipt prefix drift");
if (fixture.cases.length !== 3 || fixture.cases.some((candidate: any) => candidate.bindings.length !== 6 || candidate.sealedObservations.length !== 6)) throw new Error("Wave23 fixture cardinality drift");
...
for (const [path, value] of [[harnessPath, hashes.harness], [fixturePath, hashes.fixture], [targetPath, hashes.target]] as const) if (sha256CanonicalText(read(path)) !== value) throw new Error(`Wave23 evidence/worktree drift: ${path}`);
...
for (const candidate of fixture.cases) {
  ...
  for (const source of runtimeSourceHashes) if (sha256CanonicalText(read(source.path)) !== source.sha256) throw new Error(`Wave23 runtime source drift: ${source.path}`);
  for (const binding of candidate.bindings) {
    const directory = mkdtempSync(join(tmpdir(), "wave23-mutation-receipt-"));
    try {
      ...
      writeFileSync(inputPath, JSON.stringify({ binding: { ...binding, fixtureId: fixture.fixtureId, harnessId: "harness:wave23:mutations" }, fixturePayload: fixture, evidenceCommit, runtimeSourceHashes, invocation }), "utf8");
      ...
      if (!oracle) throw new Error("Wave23 oracle missing");
      const expectedResult = JSON.stringify(projectObjectDbMutationOracleResult(oracle));
      if (reply !== "NO_REPLY" || result !== expectedResult) throw new Error(`Wave23 harness mismatch: ${binding.scenarioKind}`);
      ...
      const payload: Omit<ObjectDbConsumerExecutionReceipt, "receiptSha256"> = {
        receiptId: `receipt:wave23:${candidate.consumerId}:${binding.scenarioKind.toLowerCase()}`, consumerId: candidate.consumerId, proofMode: "DIRECT",
        harness: { harnessId: "harness:wave23:mutations", harnessCaseId: binding.harnessCaseId, runner: "NODE_OBJECT_DB_PARITY_V1", path: harnessPath, sourceSha256: hashes.harness },
...
if (added.length !== 18 || receipts.length !== 243 || new Set(receipts.map((receipt: { receiptId: string }) => receipt.receiptId)).size !== 243) throw new Error("Wave23 receipt cardinality drift");
...
console.log(JSON.stringify({ status: "PASS", preservedWave22: 225, addedWave23: 18, total: 243, evidenceCommit }));
