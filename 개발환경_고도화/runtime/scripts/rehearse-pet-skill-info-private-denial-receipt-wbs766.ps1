...
$temporaryRoot = [IO.Path]::GetFullPath((Join-Path $worktreeRoot ".tmp\wbs766-private-denial-mariadb"))
...
$port = 3336
$databaseName = "hoibot_wbs766_private_denial"
$password = "wbs766-isolated-root-only"
$serverProcess = $null
...
function Assert-SafeRoot { if (-not $temporaryRoot.StartsWith($worktreeRoot + [IO.Path]::DirectorySeparatorChar,[StringComparison]::OrdinalIgnoreCase)) { throw "Unsafe WBS766 temporary path" } }
function Invoke-Checked([scriptblock]$command) { & $command; if ($LASTEXITCODE -ne 0) { throw "WBS766 child command failed: $LASTEXITCODE" } }
function Start-Isolated {
  ...
  for ($attempt=0; $attempt -lt 150; $attempt+=1) { if ($script:serverProcess.HasExited) { throw "WBS766 MariaDB exited early" }; if (Get-NetTCPConnection -State Listen -LocalPort $port -ErrorAction SilentlyContinue) { return }; Start-Sleep -Milliseconds 200 }
  throw "WBS766 MariaDB startup timeout"
}
...
try {
  Assert-SafeRoot
  if (Get-NetTCPConnection -State Listen -LocalPort $port -ErrorAction SilentlyContinue) { throw "WBS766 port already in use" }
  ...
  $env:DATABASE_ENABLED="true"; $env:DATABASE_HOST="127.0.0.1"; $env:DATABASE_PORT=[string]$port; $env:DATABASE_USER="root"; $env:DATABASE_PASSWORD=$password; $env:DATABASE_NAME=$databaseName; $env:HOIBOT_ENVIRONMENT_CODE="dev"; $env:IRIS_SHARED_TOKEN="wbs766-isolated-token"; $env:USER_VERIFICATION_PEPPER="wbs766-isolated-pepper"
  Push-Location $runtimeRoot
  try { Invoke-Checked { & node --import tsx scripts/migrate.ts }; Invoke-Checked { & node --import tsx scripts/probe-pet-skill-info-private-denial-receipt-synthetic.ts } } finally { Pop-Location }
  Stop-Isolated; Start-Isolated
  Push-Location $runtimeRoot
  try { Invoke-Checked { & node --import tsx scripts/probe-pet-skill-info-private-denial-receipt-synthetic.ts --verify-restart } } finally { Pop-Location }
  ...
  Write-Output "WBS766_ISOLATED_MARIADB_PASS actualBuildApp=true commonRecovery=true deniedCompleted=true ignored=true replay=true legacyFailedCompatible=true restart=true port=$port production3306Unchanged=true"
} finally {
  Stop-Isolated
