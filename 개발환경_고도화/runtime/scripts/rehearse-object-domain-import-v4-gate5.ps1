...
$temporaryRoot = [IO.Path]::GetFullPath((Join-Path $worktreeRoot ".tmp\wbs742-v4-gate5-mariadb"))
$expectedRoot = [IO.Path]::GetFullPath((Join-Path $worktreeRoot ".tmp\wbs742-v4-gate5-mariadb"))
...
$port = 3364
$databaseName = "hoibot_rehearsal_wbs742_v4_gate5"
$password = "wbs742-v4-gate5-only"
$serverProcess = $null
$initialPid = $null
$restartPid = $null
$success = $null
...
$environmentNames = @("DATABASE_ENABLED","DATABASE_HOST","DATABASE_PORT","DATABASE_USER","DATABASE_PASSWORD","DATABASE_NAME","HOIBOT_ENVIRONMENT_CODE","IRIS_SHARED_TOKEN","WBS742_V4_GATE5_PHASE")
$savedEnvironment = @{}
foreach ($name in $environmentNames) { $savedEnvironment[$name] = [Environment]::GetEnvironmentVariable($name, "Process") }

function Assert-SafeTemporaryRoot {
  if ($temporaryRoot -ne $expectedRoot -or -not $temporaryRoot.StartsWith($worktreeRoot + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase)) { throw "Unsafe WBS742 V4 Gate5 temporary path: $temporaryRoot" }
}
...
function Start-IsolatedMariaDb {
  ...
  for ($attempt = 0; $attempt -lt 150; $attempt += 1) {
    if ($script:serverProcess.HasExited) { throw "WBS742 V4 Gate5 MariaDB exited early. See $errorLog" }
    ...
    if ($null -ne $listener) {
      ...
      if ($owners.Count -ne 1 -or $owners[0] -ne $script:serverProcess.Id) { throw "WBS742 V4 Gate5 listener ownership mismatch" }
      return $script:serverProcess
    }
    Start-Sleep -Milliseconds 200
  }
  throw "Timed out waiting for WBS742 V4 Gate5 MariaDB"
}
...
function Stop-OwnedMariaDb([Diagnostics.Process]$process) {
  ...
  if (-not $process.HasExited) {
    ...
    $owners = @($listener | Select-Object -ExpandProperty OwningProcess -Unique)
    if ($null -ne $listener -and ($owners.Count -ne 1 -or $owners[0] -ne $process.Id)) { throw "WBS742 V4 Gate5 listener owner changed" }
  ...
  throw "WBS742 V4 Gate5 listener remained after stop"
}

function Invoke-Checked([scriptblock]$command) {
  & $command
  if ($LASTEXITCODE -ne 0) { throw "WBS742 V4 Gate5 child command failed: $LASTEXITCODE" }
}

try {
  Assert-SafeTemporaryRoot
  if (Get-NetTCPConnection -State Listen -LocalPort $port -ErrorAction SilentlyContinue) { throw "WBS742 V4 Gate5 port $port is already in use" }
  ...
  if (Test-Path -LiteralPath $temporaryRoot) { throw "WBS742 V4 Gate5 temporary directory already exists: $temporaryRoot" }
  ...
  $env:IRIS_SHARED_TOKEN = "wbs742-v4-gate5-isolated-token"
  Push-Location $runtimeRoot
  try {
    Invoke-Checked { & npm.cmd run db:migrate }
    Invoke-Checked { & npm.cmd run object-data:validate }
    $env:WBS742_V4_GATE5_PHASE = "prepare"
    Invoke-Checked { & node --import tsx --test test/object-domain-import-v4-gate5-mariadb.integration.test.ts }
    Stop-OwnedMariaDb $serverProcess
    $serverProcess = Start-IsolatedMariaDb
    $restartPid = $serverProcess.Id
    if ($restartPid -eq $initialPid) { throw "WBS742 V4 Gate5 restart did not change PID" }
    $env:WBS742_V4_GATE5_PHASE = "replay-rollback"
    Invoke-Checked { & node --import tsx --test test/object-domain-import-v4-gate5-mariadb.integration.test.ts }
  ...
  $success = "WBS742_V4_GATE5_MARIADB_PASS port=$port database=$databaseName registeredTables=119 directTargets=47 columns=263 definitions=25 rows=49 comparedValues=272 fresh=true forcedRollback=true replay=true rollback=true restart=true production3306Unchanged=true"
} finally {
  ...
  } else { $cleanupErrors.Add("WBS742 V4 Gate5 listener remained after cleanup") }
  ...
  if ($cleanupErrors.Count -gt 0) { throw "WBS742 V4 Gate5 cleanup failed: $($cleanupErrors -join ' | ')" }
}
