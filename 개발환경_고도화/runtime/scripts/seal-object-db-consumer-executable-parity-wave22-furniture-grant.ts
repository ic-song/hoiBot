...
const fixturePath=resolve(repoRoot,"개발환경_고도화/migration-control/contracts/object-db-consumer-executable-parity-wave22-furniture-grant-v1.json");
const targetPath=resolve(runtimeRoot,"test/fixtures/object-db-executable-parity-wave22-furniture-grant.mjs");
const fixture=JSON.parse(readFileSync(fixturePath,"utf8"));
const contract=fixture.mutationContract;
const runRoot=mkdtempSync(join(tmpdir(),"wave22-furniture-grant-live-"));
...
async function collect():Promise<ObjectDbMutationScenarioEvidence[]>{
  ...
  const writer=pair.find(trace=>trace.committedRowCount>0),replay=pair.find(trace=>trace!==writer);if(!writer||!replay||replay.replayed!==true||replay.committedRowCount!==0)throw new Error(`Wave22 concurrency cardinality drift: ${JSON.stringify(pair)}`);
