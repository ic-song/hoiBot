...
$temporaryRoot = [IO.Path]::GetFullPath((Join-Path $worktreeRoot ".tmp\wbs787-furniture-place-mariadb"))
...
$port = 3346
$databaseName = "hoibot_wbs787_furniture_place_2615"
$password = "wbs787-isolated-root-only"
$serverProcess = $null
...
function Assert-SafeRoot { if (-not $temporaryRoot.StartsWith($worktreeRoot + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase)) { throw "Unsafe WBS787 temporary path" } }
function Invoke-Checked([scriptblock]$command) { & $command; if ($LASTEXITCODE -ne 0) { throw "WBS787 child command failed: $LASTEXITCODE" } }
function Start-Isolated {
  ...
  for ($attempt = 0; $attempt -lt 150; $attempt += 1) {
    if ($script:serverProcess.HasExited) { throw "WBS787 MariaDB exited early" }
    if ($null -ne (Get-NetTCPConnection -State Listen -LocalPort $port -ErrorAction SilentlyContinue)) { return }
    Start-Sleep -Milliseconds 200
  }
  throw "WBS787 MariaDB startup timeout"
}
...
try {
  Assert-SafeRoot
  if (Get-NetTCPConnection -State Listen -LocalPort $port -ErrorAction SilentlyContinue) { throw "WBS787 port already in use" }
  ...
  foreach ($migration in @("443_object_identity_audit_provider.sql", "444_canonical_item_inventory.sql", "445_object_furniture_home_canonical_model.sql")) {
    $migrationPath = (Join-Path $runtimeRoot "migrations\$migration").Replace("\", "/")
    Invoke-Checked { & $clientBinary --protocol=TCP --host=127.0.0.1 "--port=$port" --user=root "--password=$password" "--database=$databaseName" "--execute=SOURCE $migrationPath" }
  }
  ...
  $env:HOIBOT_ENVIRONMENT_CODE = "dev"; $env:IRIS_SHARED_TOKEN = "wbs787-isolated-token"; $env:USER_VERIFICATION_PEPPER = "wbs787-isolated-pepper"
  Push-Location $runtimeRoot
  try { Invoke-Checked { & node --import tsx scripts/seal-canonical-furniture-place-targeted.ts } } finally { Pop-Location }
  ...
  Write-Output "WBS787_TARGETED_MARIADB_PASS consumer=sql-repository-818137c4fb22037a migrations=443+444+445 scenarios=6 successDml=4 rollbackCommitted=0 duplicateDml=0 driftDml=0 restartDml=0 concurrencySingleWriter=true production3306Unchanged=true"
} finally {
  Stop-Isolated
  ...
  if ($null -ne $remaining) { throw "WBS787 listener remained" }
}
