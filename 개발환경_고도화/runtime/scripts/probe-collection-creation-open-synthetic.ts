...
import { CollectionCreationOpenService } from "../src/mini-pet/collection-creation-open-service.js";

const config = loadConfig();
if (!config.database.enabled) throw new Error("DATABASE_ENABLED must be true.");
if (config.database.name !== "hoibot_schema_design" && !/^hoibot_rehearsal_[a-z0-9_]+$/i.test(config.database.name)) {
  throw new Error(`Synthetic collection creation open probe is blocked for database: ${config.database.name}`);
}

const database = createDatabaseClient(config.database);
const eventId = "collection-creation-open-gb725t";
...
try {
      ...
      await transaction.execute(
        `DELETE owned FROM owned_mini_pets owned
         ...
         WHERE owned.player_id = 900000001 AND owned.equipped = FALSE
           AND definition_row.code = 'mini_pet_f879f4cf45f6a74d'`
      );
      await transaction.execute(
        `INSERT INTO inventory_stacks (player_id, item_id, quantity, version)
         SELECT 900000001, id, CASE code WHEN 'bag_11319869697c3c00' THEN 1 ELSE 0 END, 1
         FROM item_definitions WHERE code IN ('bag_11319869697c3c00', 'bag_3241894752b82f7a')
  ...
  const service = new CollectionCreationOpenService(database);
  const command = { externalUserId, channelId, message: "/컬렉션창조오픈", eventId };
  ...
  assert.deepEqual(
    ...
    { status: "opened", package: "0", ticket: "2000" }
  );
  assert.equal(result.data, "🐹 컬렉션창조패키지 오픈 완료!\n\n🎁 지급: [컬렉션창조 미니펫🐹] (창조 / 매력+1💕)\n🎟️ 추가지급: 미니펫뽑기🐹(/미니펫오픈) 2000개\n\n👉 /미니펫가방 으로 확인해주세요.");
  ...
  const rows = await database.query<Array<{
    ...
    `SELECT
       MAX(CASE WHEN item.code = 'bag_11319869697c3c00' THEN stack.quantity END) AS package_quantity,
       ...
       (SELECT COUNT(*) FROM owned_mini_pets owned JOIN mini_pet_definitions definition_row
         ...
         WHERE owned.player_id = 900000001 AND owned.equipped = FALSE
           AND definition_row.code = 'mini_pet_f879f4cf45f6a74d') AS mini_pet_count,
       (SELECT COUNT(*) FROM inventory_ledger ledger JOIN operations operation_row ON operation_row.id = ledger.operation_id
         WHERE operation_row.idempotency_scope = 'mini-pet.collection-creation-open:900000004'
           AND operation_row.idempotency_key = ?) AS inventory_ledger_count,
       (SELECT COUNT(*) FROM operations WHERE idempotency_scope = 'mini-pet.collection-creation-open:900000004'
         AND idempotency_key = ?) AS operation_count,
       (SELECT COUNT(*) FROM command_executions WHERE event_id = ? AND command_code = 'collection_creation_open') AS execution_count,
     ...
     WHERE stack.player_id = 900000001
       AND item.code IN ('bag_11319869697c3c00', 'bag_3241894752b82f7a')`,
  ...
  if (prepare) {
    ...
    }, { package_quantity: 0n, ticket_quantity: 2000n, mini_pet_count: 1n });
  } else {
