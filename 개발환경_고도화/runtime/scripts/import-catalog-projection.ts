...
if (manifestPath === undefined) throw new Error("USAGE: --manifest <catalog-projection-manifest.json> [--profile v1|v2|v3|v4] [--target-schema <json>] [--identity-bindings <json>] [--object-model <json>] [--rollback]");
...
const directTargets = [...new Set(schema.columns.map((column) => column.table))];
...
if (profileVersion !== "v1") { profileV2 = parseObjectDomainImportProfileV2(await readFile(resolve("../migration-control/contracts/object-domain-import-profile.v2.json"), "utf8")); effective = applyObjectDomainImportProfileV2(effective, profileV2); }
if (profileVersion === "v4") {
  const profileV4 = parseObjectDomainImportProfileV4(await readFile(resolve("../migration-control/contracts/object-domain-import-profile.v4.json"), "utf8"));
  const schemaV4 = parseObjectDomainImportTargetSchemaV4(await readFile(resolve(`../migration-control/contracts/${profileV4.targetSchemaAmendment}`), "utf8"));
  effective = applyObjectDomainImportProfileV4(effective, schemaV4);
}
...
const policy: CatalogProjectionPolicy = {
  targetSchemaSha256: profileVersion === "v1" ? calculateCatalogTargetSchemaSha256(schemaBytes.toString("utf8")) : calculateCatalogTargetSchemaSha256(effectiveSchemaText),
  legacyTargetSchemaSha256s: profileVersion === "v1" ? calculateLegacyCatalogTargetSchemaSha256s(schemaBytes.toString("utf8")) : [],
