...
const harnessHash=sha256CanonicalText(read(harnessPath)),fixtureHash=sha256CanonicalText(read(fixturePath)),targetHash=sha256CanonicalText(read(targetPath)),dmlHash=sha256CanonicalJson({normalizedStatements:[],rowCount:0});
...
for(const binding of fixture.bindings){
  ...
  if(!consumer||!source||source.kind!=="RUNTIME_DISPATCH"||source.access!=="READ"||source.unresolvedDynamicCallCount!==0||entry.verdict!=="STATIC_ONLY")throw new Error("Wave7 source cohort drift");
