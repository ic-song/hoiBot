...
for(const binding of fixture.bindings){
  ...
  const result=negative?JSON.stringify({error:reply,queryCount:0,mutationCount:0}):binding.scenarioKind==="RESTART_CONSISTENCY"?JSON.stringify({result:expected,restartEvidence:{processExecutions:2,distinctProcessIds:true,distinctModuleExecutions:true}}):reply;
