...
if (!/^hoibot_data_restore(?:_[a-z0-9_]+)?$/i.test(config.database.name)) {
  throw new Error(
    `Synthetic data-restore probe blocked: ${config.database.name}`,
  );
}
const db = createDatabaseClient(config.database);
const restart = process.argv.includes("--verify-restart");
const user = "synthetic-data-restore-manager";
const room = "synthetic-data-restore-room";
const prefix =
  process.env.DATA_RESTORE_PROBE_EVENT_ID ?? "data-restore-g7-20260828-r1";

try {
  const service = new DataRestoreService(db);
  if (!restart) {
    const operator = 988700010;
    await db.execute(
      "INSERT INTO admin_operators(id,login_id,display_name,password_hash,status) VALUES (?,'synthetic-data-restore','합성 데이터복구 운영자','synthetic','active') ON DUPLICATE KEY UPDATE status='active'",
      [operator],
    );
    await db.execute(
    ...
    await source("prod", "synthetic-restore-revision-1", "member", "backup1", '{"value":"restored"}');
    await managed("prod", "member.json", '{"value":"before"}');
    ...
    const before = await count(
      "SELECT revision_version value FROM managed_data_objects WHERE environment_code='prod' AND file_name='member.json'",
    );
    assert.deepEqual(
    ...
    await source("prod", "synthetic-restore-rollback", "member_pet", "backup1", '{"value":"rollback"}');
    await managed("prod", "member_pet.json", '{"value":"safe"}');
    ...
    await assert.rejects(
      ...
      /synthetic data restore audit failure/,
    );
    const rollbackValue = (
    ...
    assert.equal(rollbackValue, '{"value":"safe"}');
    console.log(
      JSON.stringify({
        mode: "probe",
        migrationCount: 272,
        scenarios: [
          ...
          "strict-json-hash-size",
          "pre-restore-snapshot",
          "replay",
          "shadow",
          "rollback",
        ],
        effects: { operations: 1, restores: 1, snapshots: 1, outboxes: 1 },
        operationalDataTouched: false,
      }),
    );
  } else {
    const before = await count(
      "SELECT COUNT(*) value FROM operations WHERE idempotency_scope='data_restore.execute'",
    );
    const result = await service.restore({
      eventId: `${prefix}-run`,
      externalUserId: user,
        ...
        work({
          ...
          execute: async (sql, values) => {
            if (sql.includes("INSERT INTO command_audit")) {
              throw new Error("synthetic data restore audit failure");
            }
