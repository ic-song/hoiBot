...
$temporaryRoot = [IO.Path]::GetFullPath((Join-Path $worktreeRoot ".tmp\wave9-executable-parity-mariadb"))
$expectedRoot = [IO.Path]::GetFullPath((Join-Path $worktreeRoot ".tmp\wave9-executable-parity-mariadb"))
...
$port = 3330
$databaseName = "hoibot_wave9_executable_parity"
$password = "wave9-isolated-root-only"
$serverProcess = $null
$success = $null
...
$names = @("DATABASE_ENABLED", "DATABASE_HOST", "DATABASE_PORT", "DATABASE_USER", "DATABASE_PASSWORD", "DATABASE_NAME", "HOIBOT_ENVIRONMENT_CODE", "IRIS_SHARED_TOKEN", "USER_VERIFICATION_PEPPER", "WAVE9_ISOLATED_MARIADB_TEST")
$saved = @{}
...
function Assert-SafeRoot {
  if ($temporaryRoot -ne $expectedRoot -or -not $temporaryRoot.StartsWith($worktreeRoot + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase)) {
    throw "Unsafe Wave9 temporary path: $temporaryRoot"
  }
}
function Start-Isolated {
  ...
  for ($attempt = 0; $attempt -lt 150; $attempt += 1) {
    if ($script:serverProcess.HasExited) { throw "Wave9 MariaDB exited early" }
    $listener = Get-NetTCPConnection -State Listen -LocalPort $port -ErrorAction SilentlyContinue
    if ($null -ne $listener) {
      $owners = @($listener | Select-Object -ExpandProperty OwningProcess -Unique)
      if ($owners.Count -ne 1 -or $owners[0] -ne $script:serverProcess.Id) { throw "Wave9 listener ownership mismatch" }
      return
    }
    Start-Sleep -Milliseconds 200
  }
  throw "Wave9 MariaDB startup timeout"
}
function Stop-Isolated {
  ...
  throw "Wave9 listener did not clear"
}
function Invoke-Checked([scriptblock]$command) {
  & $command
  if ($LASTEXITCODE -ne 0) { throw "Wave9 child command failed: $LASTEXITCODE" }
}
...
try {
  ...
  $env:HOIBOT_ENVIRONMENT_CODE = "dev"; $env:IRIS_SHARED_TOKEN = "wave9-isolated-token"; $env:USER_VERIFICATION_PEPPER = "wave9-isolated-pepper"; $env:WAVE9_ISOLATED_MARIADB_TEST = "true"
  Push-Location $runtimeRoot
  try {
    Invoke-Checked { & node --import tsx scripts/migrate.ts }
    Invoke-Checked { & node --import tsx --test test/object-db-consumer-executable-parity-wave9-mariadb.integration.test.ts }
  ...
  $success = "WAVE9_ISOLATED_MARIADB_PASS consumers=5 concurrency=5 rollback=5 pendingForUpdateObserved=true oneWriterReplay=true persistedRollbackZero=true production3306Unchanged=true"
} finally {
  ...
  if ($null -ne $remaining) { $errors.Add("Wave9 listener remained") }
  ...
  if ($errors.Count -gt 0) { throw "Wave9 cleanup failed: $($errors -join ' | ')" }
}
