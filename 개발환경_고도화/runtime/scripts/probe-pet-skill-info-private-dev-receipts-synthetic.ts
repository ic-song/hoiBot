...
async function main(){
  ...
  try{
    ...
    const prodApp=await application(database,"prod","group");try{const rejected=await send(prodApp,"wbs765-prod-dev-rejected","dev/펫스킬정보");assert.equal(rejected.statusCode,500,rejected.body);assert.equal((await operationRows(database,"wbs765-prod-dev-rejected"))[0]!.result_code,"PET_SKILL_INFO_DEV_ENVIRONMENT_REQUIRED");}finally{await prodApp.close();database=open();}
