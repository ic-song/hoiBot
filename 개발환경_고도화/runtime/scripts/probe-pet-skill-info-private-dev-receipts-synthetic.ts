...
async function main(){
  ...
  try{
    ...
    const deniedApp=await application(database,"dev","private");try{const denied=await send(deniedApp,"wbs765-private-denied","/펫스킬정보","wbs765-denied-user");assert.equal(denied.statusCode,202,denied.body);assert.deepEqual(JSON.parse(denied.body),{ok:true,accepted:true,ignored:true,ignoreReason:"PET_SKILL_INFO_PRIVATE_PASS_REQUIRED",requestId:"iris:wbs765-private-denied"});const row=(await operationRows(database,"wbs765-private-denied"))[0]!;assert.equal(row.operation_status,"failed");assert.equal(row.result_code,"PET_SKILL_INFO_PRIVATE_PASS_REQUIRED");}finally{await deniedApp.close();database=open();}
    ...
    const prodApp=await application(database,"prod","group");try{const rejected=await send(prodApp,"wbs765-prod-dev-rejected","dev/펫스킬정보");assert.equal(rejected.statusCode,202,rejected.body);assert.deepEqual(JSON.parse(rejected.body),{ok:true,accepted:true,ignored:true,ignoreReason:"PET_SKILL_INFO_DEV_ENVIRONMENT_REQUIRED",requestId:"iris:wbs765-prod-dev-rejected"});assert.equal((await operationRows(database,"wbs765-prod-dev-rejected")).length,0);}finally{await prodApp.close();database=open();}
