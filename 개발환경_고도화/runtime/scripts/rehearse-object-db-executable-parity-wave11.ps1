...
$temporaryRoot = [IO.Path]::GetFullPath((Join-Path $worktreeRoot ".tmp\wave11-executable-parity-mariadb"))
$expectedRoot = [IO.Path]::GetFullPath((Join-Path $worktreeRoot ".tmp\wave11-executable-parity-mariadb"))
...
$port = 3330
$databaseName = "hoibot_wave11_executable_parity"
$password = "wave11-isolated-root-only"
$serverProcess = $null
$success = $null
...
$names = @("DATABASE_ENABLED", "DATABASE_HOST", "DATABASE_PORT", "DATABASE_USER", "DATABASE_PASSWORD", "DATABASE_NAME", "HOIBOT_ENVIRONMENT_CODE", "IRIS_SHARED_TOKEN", "USER_VERIFICATION_PEPPER", "WAVE11_ISOLATED_MARIADB_TEST")
$saved = @{}
foreach ($name in $names) { $saved[$name] = [Environment]::GetEnvironmentVariable($name, "Process") }
function Assert-SafeRoot { if ($temporaryRoot -ne $expectedRoot -or -not $temporaryRoot.StartsWith($worktreeRoot + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase)) { throw "Unsafe Wave11 temporary path: $temporaryRoot" } }
function Start-Isolated {
  ...
  for ($attempt = 0; $attempt -lt 150; $attempt += 1) { if ($script:serverProcess.HasExited) { throw "Wave11 MariaDB exited early" }; $listener = Get-NetTCPConnection -State Listen -LocalPort $port -ErrorAction SilentlyContinue; if ($null -ne $listener) { $owners = @($listener | Select-Object -ExpandProperty OwningProcess -Unique); if ($owners.Count -ne 1 -or $owners[0] -ne $script:serverProcess.Id) { throw "Wave11 listener ownership mismatch" }; return }; Start-Sleep -Milliseconds 200 }
  throw "Wave11 MariaDB startup timeout"
}
function Stop-Isolated { if ($null -eq $script:serverProcess) { return }; if (-not $script:serverProcess.HasExited) { Stop-Process -Id $script:serverProcess.Id; $script:serverProcess.WaitForExit(10000) | Out-Null }; for ($attempt = 0; $attempt -lt 50; $attempt += 1) { if ($null -eq (Get-NetTCPConnection -State Listen -LocalPort $port -ErrorAction SilentlyContinue)) { return }; Start-Sleep -Milliseconds 200 }; throw "Wave11 listener did not clear" }
function Invoke-Checked([scriptblock]$command) { & $command; if ($LASTEXITCODE -ne 0) { throw "Wave11 child command failed: $LASTEXITCODE" } }
try {
  Assert-SafeRoot
  if (Get-NetTCPConnection -State Listen -LocalPort $port -ErrorAction SilentlyContinue) { throw "Wave11 port already in use" }
  ...
  if (Test-Path -LiteralPath $temporaryRoot) { throw "Wave11 temporary directory exists" }
  ...
  $env:DATABASE_ENABLED="true"; $env:DATABASE_HOST="127.0.0.1"; $env:DATABASE_PORT=[string]$port; $env:DATABASE_USER="root"; $env:DATABASE_PASSWORD=$password; $env:DATABASE_NAME=$databaseName; $env:HOIBOT_ENVIRONMENT_CODE="dev"; $env:IRIS_SHARED_TOKEN="wave11-isolated-token"; $env:USER_VERIFICATION_PEPPER="wave11-isolated-pepper"; $env:WAVE11_ISOLATED_MARIADB_TEST="true"
  Push-Location $runtimeRoot
  try { Invoke-Checked { & node --import tsx scripts/migrate.ts }; Invoke-Checked { & node --import tsx --test test/object-db-consumer-executable-parity-wave11-mariadb.integration.test.ts } } finally { Pop-Location }
  ...
  $success="WAVE11_ISOLATED_MARIADB_PASS consumers=2 actualHttpIngress=true concurrency=2 rollback=2 sourceSignatures=11 inboxVsOperationReplay=true linkageGapObserved=true production3306Unchanged=true"
}
finally {
  ...
  if ($null -ne $remaining) { $errors.Add("Wave11 listener remained") }
  ...
  if ($errors.Count -gt 0) { throw "Wave11 cleanup failed: $($errors -join ' | ')" }
}
