...
function Assert-ExactTemporaryPath {
  if ($temporaryRoot -ne $expectedTemporaryRoot -or -not $temporaryRoot.StartsWith($worktreeRoot + [System.IO.Path]::DirectorySeparatorChar, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "Unsafe Gate5 temporary path: $temporaryRoot"
  }
}
...
function Start-IsolatedMariaDb {
  ...
  if ($ForceStartupFailure) { throw "WBS742_GATE5_FORCED_STARTUP_FAILURE" }
  for ($attempt = 0; $attempt -lt 150; $attempt += 1) {
    ...
    if ($null -ne $listener) {
      ...
      if ($owners.Count -ne 1 -or $owners[0] -ne $process.Id) { throw "Gate5 listener ownership mismatch." }
      return $process
    }
...
function Stop-OwnedMariaDb([System.Diagnostics.Process]$process) {
  ...
  if (-not $process.HasExited) {
    ...
    if ($null -ne $listener -and ($owners.Count -ne 1 -or $owners[0] -ne $process.Id)) {
      ...
      throw "Gate5 listener ownership changed before owned process cleanup."
    }
    Stop-Process -Id $process.Id
    $process.WaitForExit(10000) | Out-Null
  ...
  for ($attempt = 0; $attempt -lt 50; $attempt += 1) {
    ...
    if ($remainingOwners.Count -ne 1 -or $remainingOwners[0] -ne $process.Id) { throw "Gate5 listener ownership changed while stopping." }
    Start-Sleep -Milliseconds 200
  }
  throw "Gate5 listener did not clear after owned process exit."
}

function Invoke-Checked([scriptblock]$command) {
  & $command
  if ($LASTEXITCODE -ne 0) { throw "Gate5 child command failed with exit code $LASTEXITCODE" }
}

try {
  Assert-ExactTemporaryPath
  if (Get-NetTCPConnection -State Listen -LocalPort $rehearsalPort -ErrorAction SilentlyContinue) { throw "Gate5 port $rehearsalPort is already in use." }
  ...
  if (Test-Path -LiteralPath $temporaryRoot) { throw "Gate5 temporary directory already exists: $temporaryRoot" }
  ...
  $env:IRIS_SHARED_TOKEN = "wbs742-gate5-isolated-token"
  ...
  if ($null -ne $remainingListener) { $cleanupErrors.Add("Gate5 listener remained after cleanup.") }
  ...
  if ($cleanupErrors.Count -gt 0) { throw "Gate5 cleanup failed: $($cleanupErrors -join ' | ')" }
}
