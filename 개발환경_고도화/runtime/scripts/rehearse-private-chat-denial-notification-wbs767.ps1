...
$temporaryRoot = [IO.Path]::GetFullPath((Join-Path $worktreeRoot ".tmp\wbs767-private-denial-notice-mariadb"))
...
$port = 3337
$databaseName = "hoibot_wbs767_private_notice"
$password = "wbs767-isolated-root-only"
$serverProcess = $null
...
function Assert-SafeRoot { if (-not $temporaryRoot.StartsWith($worktreeRoot + [IO.Path]::DirectorySeparatorChar,[StringComparison]::OrdinalIgnoreCase)) { throw "Unsafe WBS767 temporary path" } }
function Invoke-Checked([scriptblock]$command) { & $command; if ($LASTEXITCODE -ne 0) { throw "WBS767 child command failed: $LASTEXITCODE" } }
function Start-Isolated {
  ...
  for ($attempt=0; $attempt -lt 150; $attempt+=1) { if ($script:serverProcess.HasExited) { throw "WBS767 MariaDB exited early" }; if (Get-NetTCPConnection -State Listen -LocalPort $port -ErrorAction SilentlyContinue) { return }; Start-Sleep -Milliseconds 200 }
  throw "WBS767 MariaDB startup timeout"
}
...
try {
  Assert-SafeRoot
  if (Get-NetTCPConnection -State Listen -LocalPort $port -ErrorAction SilentlyContinue) { throw "WBS767 port already in use" }
  ...
  $env:DATABASE_ENABLED="true"; $env:DATABASE_HOST="127.0.0.1"; $env:DATABASE_PORT=[string]$port; $env:DATABASE_USER="root"; $env:DATABASE_PASSWORD=$password; $env:DATABASE_NAME=$databaseName; $env:HOIBOT_ENVIRONMENT_CODE="dev"; $env:IRIS_SHARED_TOKEN="wbs767-isolated-token"; $env:USER_VERIFICATION_PEPPER="wbs767-isolated-pepper"
  Push-Location $runtimeRoot
  try { Invoke-Checked { & node --import tsx scripts/migrate.ts }; Invoke-Checked { & node --import tsx scripts/probe-private-chat-denial-notification-wbs767.ts } } finally { Pop-Location }
  Stop-Isolated; Start-Isolated
  Push-Location $runtimeRoot
  try { Invoke-Checked { & node --import tsx scripts/probe-private-chat-denial-notification-wbs767.ts --verify-restart } } finally { Pop-Location }
  ...
  Write-Output "WBS767_ISOLATED_MARIADB_PASS count5=true environmentIsolation=true concurrentOrdinal12=true replayIncrement0=true thirdOutbox1=true exactLegacyMessage=true crashGapRecovered=true retryExhaustionRollback=true restart=true externalSend0=true port=$port production3306Unchanged=true"
} finally {
  Stop-Isolated
