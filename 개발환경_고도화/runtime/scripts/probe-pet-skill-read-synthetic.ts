...
const base = process.env.PET_SKILL_READ_PROBE_EVENT_ID ?? "pet-skill-read-aggregate-g7-20260827-r1";
...
const read = (id: string, message: string, client = service) => client.read({ eventId: id, externalUserId: "skill-viewer", destinationId: "synthetic-pet-skill-room", message });

try {
  const success = `${base}-status`;
  if (restart) {
    ...
    await database.execute("INSERT INTO players(id,status) VALUES (987100001,'active'),(987100004,'active')");
    await database.execute("INSERT INTO player_profiles(player_id,current_display_name) VALUES (987100001,'펫스킬테스터'),(987100004,'다른유저')");
    await database.execute("INSERT INTO external_identities(id,player_id,provider_code,external_user_id,display_name,status) VALUES (987100001,987100001,'kakao','skill-viewer','펫스킬테스터','linked')");
    await database.execute("INSERT INTO player_pets(id,player_id,display_name,image_value,experience) VALUES (987100001,987100001,'호이','🐶',0),(987100004,987100004,'타인펫','🐱',0)");
    await database.execute("INSERT INTO player_pet_intimacy(player_pet_id,intimacy_level,progress,charm) VALUES (987100001,9007199254740993,0,0),(987100004,100,0,0)");
    await database.execute("INSERT INTO skill_definitions(id,code,display_name,rules_json,active) VALUES (987100002,'pet_skill_textbook','펫스킬 학개론📙',JSON_OBJECT('grade','SS','rate',1.5,'effect','슬롯 증가','tierInfo',''),TRUE),(987100003,'pet_skill_heal','회복',JSON_OBJECT('grade','A','rate',98.5,'effect','회복 효과','tierInfo','\\n티어: 1'),TRUE)");
    ...
    const aliases = await database.query<Array<{ command_text: string }>>("SELECT command_text FROM command_aliases WHERE command_code='PET_SKILL_READ' AND active=1 ORDER BY command_text");
    assert.equal(aliases.length, 4);
    await event(success);
    const status = await read(success, "/펫스킬");
    assert.equal(status.slotLimit, 33);
    assert.match(status.reply, /회복 x9007199254740993/);
    assert.deepEqual(await read(success, "/펫스킬"), status);
    const probabilityId = `${base}-probability`; await event(probabilityId);
    assert.match((await read(probabilityId, "/펫스킬확률")).reply, /총 확률: 100\.0%/);
    const infoId = `${base}-info`; await event(infoId);
    assert.match((await read(infoId, "/펫스킬정보 회복")).reply, /등급: A\n확률: 98\.5%\n효과: 회복 효과/);
    const deniedId = `${base}-denied`; await event(deniedId);
    assert.equal((await read(deniedId, "/펫스킬정보 다른유저")).reply, "❌ 다른 유저의 펫스킬 조회는 관리자만 가능합니다.");
    const rollback = `${base}-rollback`; await event(rollback);
    await assert.rejects(() => read(rollback, "/펫스킬확률", new PetSkillReadService(failAudit(database))), /synthetic pet skill read audit failure/);
    ...
    process.stdout.write(JSON.stringify({ mode: "probe", commands: 3, scenarios: ["status", "probability", "skill-info", "user-info-denied", "slot30-33", "bigint", "replay", "rollback", "no-domain-mutation"], effects: { operations: 4, outboxes: 4, rollbackOperation: 0, inventoryQuantity: "9007199254740993" }, operationalDataTouched: false }) + "\n");
  }
} finally {
