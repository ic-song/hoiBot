...
const consumerIds=new Set(["sql-repository-19f17500144188bf","sql-repository-2415b4267e1577c6","sql-repository-3b23c2f0f5501988"]),harnessId="harness:wave17:item-bag-read-providers";
...
if(fixture.consumerIds.length!==3||fixture.bindings.length!==15||new Set(fixture.bindings.map(binding=>`${binding.consumerId}:${binding.scenarioKind}`)).size!==15||fixture.consumerIds.some(id=>!consumerIds.has(id)))throw new Error("Wave17 fixture consumer/scenario drift");
...
for(const sourceBinding of fixture.bindings){
  ...
  try{
    const harnessCaseId=sourceBinding.consumerId==="sql-repository-19f17500144188bf"?"case:wave17:legacy-bag-owner-label":sourceBinding.consumerId==="sql-repository-2415b4267e1577c6"?"case:wave17:canonical-item-bag-import-readiness":"case:wave17:legacy-rank-label-side-effect-readiness";
...
if(added.length!==15||receipts.length!==182||new Set(receipts.map(receipt=>receipt.receiptId)).size!==182)throw new Error("Wave17 active receipt cardinality drift");
...
console.log(JSON.stringify({status:"PASS",preservedWave16:167,addedWave17:15,total:182,evidenceCommit}));
