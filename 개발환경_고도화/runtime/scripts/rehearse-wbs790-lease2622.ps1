...
$temporaryRoot = [IO.Path]::GetFullPath((Join-Path $worktreeRoot '.tmp/wbs790-lease2622-run2'))
$evidenceRoot = Join-Path $runtimeRoot '../migration-control/evidence/wbs790-gate6-lease2622/run2'
...
try {
  ...
  Start-OwnedServer
  & (Join-Path $mariaBin 'mariadb.exe') --protocol=TCP --host=127.0.0.1 "--port=$port" --user=root --password=lease2622-isolated-only '--execute=CREATE DATABASE hoibot_wbs790_lease2622 CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci'
  ...
  $env:DATABASE_USER='root'; $env:DATABASE_PASSWORD='lease2622-isolated-only'; $env:DATABASE_NAME='hoibot_wbs790_lease2622'
  ...
  try {
    ...
    Invoke-Checked { & node --import tsx scripts/rehearse-wbs790-lease2622.ts mutation } 'mutation.txt'
    Stop-OwnedServer
    Start-OwnedServer
    Invoke-Checked { & node --import tsx scripts/rehearse-wbs790-lease2622.ts restart-shadow } 'restart-shadow.txt'
