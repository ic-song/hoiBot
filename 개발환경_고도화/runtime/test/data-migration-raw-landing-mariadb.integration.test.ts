...
integration("data migration RAW landing MariaDB integration", () => {
  ...
  it("imports byte-exact payloads, replays with zero inserts and rolls back by run", async () => {
    ...
    const rows = await database.query<Array<{ count_value: bigint; bytes_value: bigint }>>("SELECT COUNT(*) count_value,COALESCE(SUM(size_bytes),0) bytes_value FROM data_migration_raw_files WHERE run_id=?", [BigInt(first.runId)]);
    assert.equal(Number(rows[0]!.count_value), manifest.fileCount);
    assert.equal(Number(rows[0]!.bytes_value), manifest.totalBytes);
    assert.equal(await repository.rollbackRun(manifest.bundleSha256), 1);
    const remaining = await database.query<Array<{ count_value: bigint }>>("SELECT COUNT(*) count_value FROM data_migration_raw_files WHERE run_id=?", [BigInt(first.runId)]);
