...
export async function executeWave11HomeFurnitureRead(args){
 ...
 const recordAttempt=sql=>{const normalized=String(sql).replace(/\s+/g," ").trim(),table=/^(?:INSERT\s+INTO|UPDATE|DELETE\s+FROM)\s+([A-Za-z0-9_]+)/i.exec(normalized)?.[1];if(table!==undefined&&SOURCE_TABLES.has(table))domainMutations.push(normalized);return normalized;};
