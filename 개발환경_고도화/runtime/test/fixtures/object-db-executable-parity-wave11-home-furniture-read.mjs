...
const SOURCE_TABLES=new Set(["players","player_profiles","player_legacy_rank_profiles","furniture_inventory_instances","furniture_definitions","player_homes","player_passes","player_pets","pet_skills","skill_definitions","home_furniture_equip_policy"]);
...
export async function executeWave11HomeFurnitureRead(args){
 ...
 const callsite=consumer.serviceCallsite,callsiteSource=canonical(resolve(root,callsite.file)),callsiteLine=callsiteSource.split("\n")[callsite.line-1]??"",escapedClass=callsite.className.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),escapedFile=callsite.file.split(/[\\/]/).at(-1).replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),callsitePattern=new RegExp(`^\\s*at ${escapedClass}\\.${callsite.methodName} \\(.+[\\\\/]${escapedFile}:${callsite.line}:\\d+\\)\\s*$`);
 check(callsite.className===target[2]&&callsite.methodName==="read"&&callsiteLine.includes("withTransaction"),"Wave11 canonical service callsite drift");
 const serviceModule=await tsImport(pathToFileURL(resolve(root,`개발환경_고도화/runtime/src/${target[0].replace(/\.ts$/,".js")}`)).href,import.meta.url),Service=serviceModule[target[2]],guard=serviceModule[target[1]],input=consumer.scenarioInputsByScenario[args.scenarioKind];
 ...
 const originalQuery=args.database.query.bind(args.database),originalExecute=args.database.execute.bind(args.database),originalWithTransaction=args.database.withTransaction.bind(args.database);
 const recordAttempt=sql=>{const normalized=String(sql).replace(/\s+/g," ").trim(),table=/(?:^|\s)(?:INSERT\s+INTO|REPLACE\s+INTO|UPDATE|DELETE\s+FROM|MERGE\s+INTO|TRUNCATE(?:\s+TABLE)?|ALTER\s+TABLE|DROP\s+TABLE|CREATE\s+TABLE)\s+([A-Za-z0-9_]+)/i.exec(normalized)?.[1];if(table!==undefined&&SOURCE_TABLES.has(table))domainMutations.push(normalized);return normalized;};
 args.database.query=async(sql,values=[])=>{recordAttempt(sql);return originalQuery(sql,values);};
 args.database.execute=async(sql,values=[])=>{const normalized=recordAttempt(sql),result=await originalExecute(sql,values),table=/^INSERT\s+INTO\s+([A-Za-z0-9_]+)/i.exec(normalized)?.[1];if(table!==undefined&&evidenceTables.has(table))evidenceRows[table]+=Number(result.affectedRows);return result;};
 args.database.withTransaction=async(work)=>{const state={...evidenceRows},frames=(new Error("Wave11 transaction entry").stack??"").split(/\r?\n/),namedFrames=frames.filter(frame=>frame.includes(`${callsite.className}.${callsite.methodName}`)),exactFrames=namedFrames.filter(frame=>callsitePattern.test(frame));if(namedFrames.length>0&&exactFrames.length!==1)throw new Error("Wave11 production service callsite stack drift");if(exactFrames.length===1)serviceInvocationCount+=1;try{return await originalWithTransaction(transaction=>work(transaction));}catch(error){Object.assign(evidenceRows,state);throw error;}};
 ...
 }finally{args.database.query=originalQuery;args.database.execute=originalExecute;args.database.withTransaction=originalWithTransaction;}
 ...
 check((body.accepted??false)===expected.accepted&&(body.ignored??false)===expected.ignored&&(body.channelMode??null)===expected.channelMode&&(body.duplicate??false)===expected.duplicate,"Wave11 HTTP semantics drift");
