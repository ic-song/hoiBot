...
export async function executeWave11HomeFurnitureRead(args){
 ...
 args.database.query=async(sql,values=[])=>{const normalized=String(sql).replace(/\s+/g," ").trim();if((args.consumerId==="runtime-dispatch-722afb15e9cbd92c"&&normalized.includes("admin_operator_external_identities"))||(args.consumerId==="runtime-dispatch-c3cb8d18613c2358"&&normalized.includes("idempotency_scope='home.furniture_stats_read'")))serviceInvocationCount+=1;return originalQuery(sql,values);};
