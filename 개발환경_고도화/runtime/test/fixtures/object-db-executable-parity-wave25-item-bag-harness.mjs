...
async function runWorker(inputPath, targetPath) {
  ...
  assert(hash(readFileSync(targetPath, "utf8")) === input.invocation.targetSourceSha256, "Wave25 target hash drift");
  ...
  for (const source of input.runtimeSourceHashes ?? []) {
    ...
    assert(hash(committed) === source.sha256, `Wave25 committed source drift: ${source.path}`);
    assert(hash(readFileSync(resolve(root, source.path), "utf8")) === source.sha256, `Wave25 worktree source drift: ${source.path}`);
  }
...
if (args[0] === "--worker") await runWorker(args[1], args[2]);
  ...
  if (args.length !== 3) throw new Error("Wave25 harness arguments missing");
