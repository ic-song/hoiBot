...
const TARGET = "object-db-executable-parity-wave31-member-title-legacy-list-target.mjs";
const CONSUMERS = new Set(["legacy-542e265c2135fb46", "legacy-d04b5224bde6be54"]);
...
function dmlTable(statement) {
  ...
  if (DML.test(statement) && match === null) throw new Error(`Wave31 unparsed DML: ${statement}`);
  return match?.[1];
}
...
function createStatefulDatabase(binding) {
  ...
  const database = {
    ...
    async query(sql, values = []) {
      ...
      let rows;
      if (n === "SELECT DATABASE() AS database_identity") rows = [{ database_identity: "wave31_synthetic" }];
      else if (n.includes("FROM command_aliases a")) {
        rows = values[0] === "/타이틀목록" ? [{ command_code: "PLAYER_TITLE_LIST_READ", handler_key: "player_title_list_read", auth_scope: "VERIFIED_USER", rollout_state: "ACTIVE" }] : [];
      ...
      } else throw new Error(`Wave31 unexpected SELECT: ${n}`);
    ...
    async execute(sql, values = []) {
      ...
      if (n.startsWith("INSERT INTO command_routing_decisions")) lastRoute = values[3] ?? null;
        ...
        // 실제 앱의 인프라 쓰기를 관측합니다.
      } else throw new Error(`Wave31 unexpected mutation: ${n}`);
...
async function runWorker(inputPath, targetPath) {
  ...
  assert(CONSUMERS.has(input.binding?.consumerId), "Wave31 consumer not allowlisted");
  assert(input.invocation?.exportName === "executeWave31MemberTitleLegacyList", "Wave31 export drift");
  assert(targetPath.endsWith(TARGET), "Wave31 target drift");
  assert(hash(readFileSync(targetPath, "utf8")) === input.invocation.targetSourceSha256, "Wave31 target hash drift");
  ...
  for (const source of input.runtimeSourceHashes ?? []) {
    ...
    assert(hash(committed) === source.sha256, `Wave31 committed source drift: ${source.path}`);
    assert(hash(readFileSync(resolve(root, source.path), "utf8")) === source.sha256, `Wave31 worktree source drift: ${source.path}`);
  }
...
async function runMain(inputPath, outputDirectory, targetPath) {
  ...
  if (results.length === 2) {
    assert(results[0].processId !== results[1].processId, "Wave31 restart reused process");
    assert(results[0].execution.moduleExecutionId !== results[1].execution.moduleExecutionId, "Wave31 restart reused module");
    assert(results[0].execution.reply === results[1].execution.reply && results[0].execution.result === results[1].execution.result, "Wave31 restart semantic drift");
  }
  const trace = deriveTrace(results, input.binding.scenarioKind);
  assert(trace.sourceDomainDmlCount === 0 && trace.queryTrace.length > 0 && trace.dmlTrace.length > 0 && trace.transactionAttempts.length > 0, "Wave31 observed trace incomplete");
  ...
  writeFileSync(join(outputDirectory, "case-result.json"), `${JSON.stringify({ format: "hoibot-object-db-consumer-parity-case-result-v1",
    passed: true, assertionCount: first.assertionCount + 6, executedConsumerId: first.executedConsumerId, executedCaseId: first.executedCaseId,
