...
describe("operation notice command boundary", () => {
  it("accepts only the three parameterized notice commands", () => {
    ...
    assert.equal(isOperationNoticeCommandCandidate("/광고 오늘의 광고"), true);
    ...
    assert.equal(isOperationNoticeCommandCandidate("/광고"), false);
    assert.equal(isOperationNoticeCommandCandidate("/광고문구"), false);
  });
  ...
  it("normalizes legacy newline tokens and preserves empty clear semantics", () => {
    ...
    assert.equal(parseOperationNoticeCommand("/광고  첫째\\n둘째  ")!.value, "첫째\n둘째");
  });
  ...
  it("normalizes argument commands for exact DB dispatch", () => {
    ...
    assert.equal(normalizeOperationNoticeDispatchMessage("/광고 안내"), "/광고");
  });
  ...
  it("rejects NUL and oversized content without exposing it", () => {
    ...
    assert.throws(() => parseOperationNoticeCommand(`/광고 ${"a".repeat(16_385)}`), (error) => error instanceof ApplicationError && error.statusCode === 422);
  });
});
