...
describe("object DB executable parity Wave7 service-chain cohort",()=>{
  it("promotes three static runtime dispatch consumers with five direct receipts each",()=>{
    ...
    assert.equal(ledger.coverage.manifestConsumers,1111);assert.equal(ledger.coverage.ledgerEntries,1111);assert.equal(ledger.coverage.provenConsumers,14);assert.equal(ledger.coverage.unprovenConsumers,1097);assert.equal(ledger.coverage.directPassConsumers,14);assert.equal(ledger.coverage.equivalentPassConsumers,0);assert.deepEqual(ledger.coverage.verdicts,{STATIC_ONLY:545,BLOCKED_DYNAMIC:552,BLOCKED_REGISTRY_MISMATCH:0,PARTIAL:0,DIRECT_PASS:14,EQUIVALENT_PASS:0});assert.equal(ledger.coverage.registrySourceMismatchCount,8);
