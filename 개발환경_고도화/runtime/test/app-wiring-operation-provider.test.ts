...
describe("MariaAppWiringOperationProvider T0 claim",()=>{
  ...
  it("declares all ten typed receipt table/id/status mappings",()=>{const source=readFileSync(new URL("../src/dispatch/app-wiring-operation-provider.ts",import.meta.url),"utf8");for(const [kind,column,table] of [["DAILY_PRAYER","daily_prayer_operation_id","canonical_daily_prayer_operations"],["HOME_AGGREGATE","home_aggregate_operation_id","canonical_home_aggregate_operations"],["MARKET","market_operation_id","canonical_market_operations"],["MEMBER_TITLE","member_title_operation_id","canonical_member_title_operations"],["MINI_PET_TITLE","mini_pet_title_operation_id","canonical_mini_pet_title_operations"],["PACKAGE_USE","package_use_operation_id","canonical_package_use_operations"],["PET_EXPLORE","pet_explore_operation_id","canonical_pet_explore_operations"],["PET_EXPLORE_EVENT_CONTROL","pet_explore_event_control_operation_id","canonical_pet_explore_event_control_operations"],["PET_TITLE","pet_title_operation_id","canonical_pet_title_operations"],["PLAYER_IDENTITY","player_identity_operation_id","canonical_player_identity_operations"]])assert.match(source,new RegExp(`${kind}: \\["${column}","${table}","operation_status"`));});
});
