...
test("residual work plan freezes every unproven consumer into one actionable category", () => {
  ...
  assert.equal(plan.summary.alreadyDirectOrEquivalent, 67);
  assert.equal(plan.summary.residualConsumers, 1_066);
  assert.deepEqual(plan.summary.categoryCounts, {
    B_STRICT_EQUIVALENCE: 0,
    C_DIRECT_EXECUTION: 984,
    D_PREREQUISITE: 82,
  });
  assert.equal(new Set(plan.entries.map((entry: { consumerId: string }) => entry.consumerId)).size, 1_066);
  for (const consumerId of ["sql-repository-818137c4fb22037a", "sql-repository-f6c531148a436a21", "sql-repository-31c4099080d9c9c1", "sql-repository-87ed81931dd7417b", "legacy-94904fa11988ff04", "legacy-827e1dc284cea52c", "legacy-bf7edb9e7cee98cd", "legacy-542e265c2135fb46", "legacy-d04b5224bde6be54", "legacy-798257cac0e93e27"]) {
