...
import { normalizeCanonicalPetSkillOptions, resolveCanonicalPetSkillCommand, resolveCanonicalPetSkillHandler } from "../src/pet/canonical-pet-skill-handler-registry.js";
...
describe("canonical pet skill handler registry", () => {
  ...
  it("accepts only allowlisted handler keys and typed options", () => {
    ...
    assert.deepEqual(normalizeCanonicalPetSkillOptions("command_unlock", { commandIdentifier: "pet_skill_boast", chancePercent: 70 }), { commandIdentifier: "pet_skill_boast", chancePercent: 70 });
    assert.deepEqual(normalizeCanonicalPetSkillOptions("presentation_only", {}), {});
    assert.deepEqual(resolveCanonicalPetSkillCommand("pet_skill_duel"), { trigger: "/결투" });
  ...
  it("rejects executable or unknown option fields and invalid values", () => {
    assert.throws(() => normalizeCanonicalPetSkillOptions("command_unlock", { commandIdentifier: "pet_skill_duel", javascript: "doWork()" }), /OPTION_KEY_NOT_ALLOWED/);
    for (const commandIdentifier of ["/관리자삭제", "eval('x')", "unregistered_command", "그냥문자열"]) assert.throws(() => normalizeCanonicalPetSkillOptions("command_unlock", { commandIdentifier }), /COMMAND_NOT_ALLOWED/);
