...
export async function buildSyntheticAdminWebShellApp() {
  ...
  app.get<{ Querystring: { query?: string; objectType?: string; active?: string; page?: string } }>("/api/v1/admin/object-catalog/objects", async (request) => {
    const query = String(request.query.query ?? "").toLowerCase();
    const items = Array.from(objects.values()).filter((object) => (!query || object.objectKey.toLowerCase().includes(query) || object.displayName.toLowerCase().includes(query)) && (!request.query.objectType || object.objectType === request.query.objectType) && (!request.query.active || String(object.active) === request.query.active));
    return { ok: true, catalog: { page: Number(request.query.page ?? 1), limit: 25, total: items.length, activeCount: items.filter((object) => object.active).length, inactiveCount: items.filter((object) => !object.active).length, items: items.map((object) => ({ ...object, sourceBindings: [{ system: "RUNTIME_DB", table: "currency_definitions", key: object.objectKey }] })), typeCounts: [{ objectType: "CURRENCY", count: items.length }], domains: [{ domain: "ITEM", canonicalTable: "item_definitions", count: 182 }, { domain: "PET_SKILL", canonicalTable: "skill_definitions", count: 93 }, { domain: "MINI_PET", canonicalTable: "mini_pet_definitions", count: 1078 }, { domain: "FURNITURE", canonicalTable: "furniture_definitions", count: 1533 }, { domain: "PENDANT_POLICY", canonicalTable: "pendant_upgrade_policy_versions", count: 1 }, { domain: "TITLE", canonicalTable: "title_definitions", count: 208 }, { domain: "PASS", canonicalTable: "support_pass_definitions", count: 7 }, { domain: "PACKAGE", canonicalTable: "package_catalog", count: 107 }], packageResolution: { catalogVersion: "ASSET-FREEZE-v2.438-package-canonical-gap-correction-01", publicationStatus: "SHADOW", packageCount: 107, rewardCount: 557, frozenResolved: 467, overlayResolved: 10, effectiveResolved: 477, residualStackGaps: 36, residualPackageGaps: 44, conflicts: 0, gaps: [{ sourceOrder: 19, targetType: "STACK", displayName: "합성 미해소 아이템", resolutionStatus: "GAP" }] } }, requestId: "synthetic-asset-catalog-list" };
  });
