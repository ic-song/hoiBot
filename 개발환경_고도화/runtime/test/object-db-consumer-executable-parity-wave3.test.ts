...
describe("object DB executable parity Wave3 player target",()=>{
 it("binds one source-derived consumer to five direct receipts",()=>{const ledger=read("개발환경_고도화/migration-control/contracts/object-db-consumer-executable-parity-ledger.v1.json"), receipts=read("개발환경_고도화/migration-control/fixtures/synthetic-relational/object-db-consumer-execution-receipts-wave3-v1.json");assert.equal(ledger.coverage.manifestConsumers,1111);assert.equal(ledger.coverage.provenConsumers,7);assert.equal(ledger.coverage.unprovenConsumers,1104);assert.equal(receipts.receipts.length,35);assert.equal(receipts.receipts.filter((x:any)=>x.consumerId===id).length,5);assert.equal(ledger.entries.find((x:any)=>x.consumerId===id).verdict,"DIRECT_PASS");assert.doesNotThrow(()=>assertTrustedWave3ConsumerFixtureMapping(id,consumer,source));});
