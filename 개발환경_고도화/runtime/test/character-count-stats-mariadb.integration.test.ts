...
integration("admin character count stats MariaDB integration", () => {
  ...
  it("validates Shadow, permission, exact bundle, replay, rollback and stable DB snapshot", async () => {
    const service=new CharacterCountStatsService(database),shadowEvent=`${prefix}-shadow`;await event(shadowEvent);
    ...
    const success=`${prefix}-success`;await event(success);const result=await new IrisAdminCommandService(database).changePlayerPoint({eventId:success,externalUserId,channelId:destinationId,message:"/글자수통계"});
    assert.equal(result.status,"changed");if(result.status==="changed"){assert.match(result.data,/펜던트: .* \/ 2명/);assert.match(result.data,/자유시장:/);}
