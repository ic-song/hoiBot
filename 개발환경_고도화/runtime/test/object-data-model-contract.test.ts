...
describe("object data model standard contract", () => {
  it("accepts the registered identity/audit, item, and furniture schema contract", () => {
    ...
    assert.equal(migration486, "486_private_chat_denial_notice.sql");
    assert.equal(migration489, "489_legacy_rank_label_side_effect_certificate.sql");
    assert.equal(contract.registeredMigrations.length, 33);
    mutableMigrations.push(migration484);
    mutableMigrations.push(migration485);
    mutableMigrations.push(migration486);
    mutableMigrations.push(migration489);
    assert.equal(contract.tables.length, 114);
