...
const parityExpectations = { projectionRowCount: 49, directTargetCount: 47, schemaFieldCount: 263, definitionTargetCount: 25, comparedFieldValueCount: 272 } as const;
...
const H = (index: number): string => hash(`wbs742-v4-gate6-${index}`);
...
async function seed(database: DatabaseClient, input: any): Promise<void> {
  const audit = ["wbs742-v4-gate6", "2026-09-09 21:00:00", "wbs742-v4-gate6", "2026-09-09 21:00:00"];
...
async function counts(database: DatabaseClient): Promise<Gate6Counts> {
...
if (process.env.WBS742_V4_GATE6_PHASE !== undefined) describe("WBS742 V4 Gate6 isolated MariaDB", () => {
  it("independently verifies the exact V4 projection and canonical targets", async () => {
    const config = loadConfig();
    assertObjectDomainImportDatabaseName(config.database.name);
    assert.deepEqual([config.database.host, config.database.port, config.database.name], ["127.0.0.1", 3365, "hoibot_rehearsal_wbs742_v4_gate6"]);
    ...
    try {
      if (process.env.WBS742_V4_GATE6_PHASE === "verify") {
        ...
        await database.withTransaction(async (transaction) => {
          await transaction.execute("SET FOREIGN_KEY_CHECKS=0");
          for (const table of [...policy.directTargets].reverse()) await transaction.execute(`DELETE FROM ${table}`);
          await transaction.execute("SET FOREIGN_KEY_CHECKS=1");
        });
        await seed(database,input);
        const first=await importer.importProjection("p1234567",policy,"wbs742-v4-gate6");
        assert.deepEqual([first.insertedCanonicalRows,first.replayed],[49,false]);
        const writesBefore = await writeCounters(database);
        await assert.rejects(database.withTransaction((transaction) => verifier.verify(transaction,"p1234567",policy,{...parityExpectations,projectionRowCount:48})),/OBJECT_DOMAIN_PARITY_PROJECTION_RUN_INVALID/);
        const result = await database.withTransaction((transaction) => verifier.verify(transaction,"p1234567",policy,parityExpectations));
        const writesAfter = await writeCounters(database);
        assert.deepEqual(writesAfter,writesBefore);
        assert.deepEqual([result.projectionRowCount,result.targetRowCount,result.directTargetCount,result.schemaFieldCount,result.comparedFieldValueCount,result.rowDiffCount],[49,49,47,263,272,0]);
        assert.equal(result.projectionSha256,result.targetSha256);
        await assert.rejects(database.withTransaction(async (transaction) => {
          await transaction.execute("UPDATE canonical_player_currency_balances SET balance_minor_amount=balance_minor_amount+1");
          await verifier.verify(transaction,"p1234567",policy,parityExpectations);
        }),/OBJECT_DOMAIN_PARITY_TARGET_ROW_DRIFT/);
        await assert.rejects(database.withTransaction(async (transaction) => {
          await transaction.execute("UPDATE data_migration_object_domain_import_records SET identity_locator_sha256=? ORDER BY import_order LIMIT 1",[H(1999)]);
          await verifier.verify(transaction,"p1234567",policy,parityExpectations);
        }),/OBJECT_DOMAIN_PARITY_IMPORT_RECORD_MISMATCH/);
        await assert.rejects(database.withTransaction(async (transaction) => {
          await transaction.execute("UPDATE data_migration_catalog_projection_runs SET projection_sha256=? WHERE catalog_projection_run_id='p1234567'",[H(1998)]);
          await verifier.verify(transaction,"p1234567",policy,parityExpectations);
        }),/OBJECT_DOMAIN_PARITY_PROJECTION_FINGERPRINT_DRIFT/);
        const restored = await database.withTransaction((transaction) => verifier.verify(transaction,"p1234567",policy,parityExpectations));
        assert.deepEqual([restored.projectionRowCount,restored.targetRowCount,restored.comparedFieldValueCount,restored.rowDiffCount],[49,49,272,0]);
        process.stdout.write(`WBS742_V4_GATE6_PARITY_PASS ${JSON.stringify({rows:49,targets:47,columns:263,definitions:25,compared:272,dmlUnchanged:true,driftsRejected:3,projectionSha256:result.projectionSha256})}\n`);
      } else throw new Error("WBS742_V4_GATE6_PHASE_INVALID");
