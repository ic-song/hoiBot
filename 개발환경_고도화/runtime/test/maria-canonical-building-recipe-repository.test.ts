...
describe("MariaCanonicalBuildingRecipeRepository", () => {
  ...
  it("atomically consumes typed item/currency inputs and grants the exact output", async () => {
    ...
    const result = await repository.execute({ actor: "tester", playerId: "player01", craftRecipeId: "recipe01", requestedCount: 2n, requestKey: "diamond-2" });
  ...
  it("replays the committed result and rejects a changed payload", async () => {
    ...
    const input = { actor: "tester", playerId: "player01", craftRecipeId: "recipe01", requestedCount: 1n, requestKey: "same-request" } as const;
  ...
  it("retries a deadlock but bounds exhaustion", async () => {
    ...
    assert.equal((await new MariaCanonicalBuildingRecipeRepository(db, ids(), 2).execute({ actor: "tester", playerId: "player01", craftRecipeId: "recipe01", requestedCount: 1n, requestKey: "retry-ok" })).replayed, false);
    const exhausted = new CraftDatabase(); exhausted.failTransactions = 2;
    await assert.rejects(() => new MariaCanonicalBuildingRecipeRepository(exhausted, ids(), 2).execute({ actor: "tester", playerId: "player01", craftRecipeId: "recipe01", requestedCount: 1n, requestKey: "retry-no" }), (error: unknown) => typeof error === "object" && error !== null && "errno" in error && error.errno === 1213);
  });
  ...
  it("replays a concurrently committed same-key request after the unique conflict", async () => {
    ...
    const result = await new MariaCanonicalBuildingRecipeRepository(db, ids(), 3).execute({ actor: "tester", playerId: "player01", craftRecipeId: "recipe01", requestedCount: 1n, requestKey: "concurrent" });
  ...
  it("retries ER_LOCK_WAIT_TIMEOUT and rejects an oversized batch before mutation", async () => {
    ...
    assert.equal((await new MariaCanonicalBuildingRecipeRepository(timeout, ids(), 2).execute({ actor: "tester", playerId: "player01", craftRecipeId: "recipe01", requestedCount: 1n, requestKey: "timeout" })).replayed, false);
    const limited = new CraftDatabase();
    await assert.rejects(() => new MariaCanonicalBuildingRecipeRepository(limited, ids()).execute({ actor: "tester", playerId: "player01", craftRecipeId: "recipe01", requestedCount: 11n, requestKey: "limit" }), /BATCH_LIMIT_EXCEEDED/);
  ...
  it("leaves balances untouched for inactive, over-limit, and insufficient recipes", async () => {
    ...
    await assert.rejects(() => new MariaCanonicalBuildingRecipeRepository(inactive, ids()).execute({ actor: "tester", playerId: "player01", craftRecipeId: "recipe01", requestedCount: 1n, requestKey: "inactive" }), /RECIPE_INACTIVE/);
    ...
    await assert.rejects(() => new MariaCanonicalBuildingRecipeRepository(insufficient, ids()).execute({ actor: "tester", playerId: "player01", craftRecipeId: "recipe01", requestedCount: 1n, requestKey: "short" }), /INSUFFICIENT_CURRENCY/);
  ...
  it("pins composite owner/target foreign keys against cross-owner ledger rows", () => {
    ...
    for (const token of ["FOREIGN KEY (craft_operation_id, player_id)", "FOREIGN KEY (owned_item_stack_id, player_id, item_id)", "FOREIGN KEY (player_currency_balance_id, player_id, currency_id)", "uq_canonical_owned_item_stack_owner_target", "request_key VARCHAR(182)"]) assert.ok(migration.includes(token));
  ...
  it("accepts the 182-character request-key boundary and fails closed for building execution", async () => {
    const db = new CraftDatabase(); const repository = new MariaCanonicalBuildingRecipeRepository(db, ids());
    await repository.execute({ actor: "tester", playerId: "player01", craftRecipeId: "recipe01", requestedCount: 1n, requestKey: "r".repeat(182) });
    await assert.rejects(() => repository.execute({ actor: "tester", playerId: "player01", craftRecipeId: "recipe01", requestedCount: 1n, requestKey: "r".repeat(183) }), /REQUEST_KEY_INVALID/);
    const building = new CraftDatabase(); building.recipe.craft_recipe_kind = "building_upgrade";
    await assert.rejects(() => new MariaCanonicalBuildingRecipeRepository(building, ids()).execute({ actor: "tester", playerId: "player01", craftRecipeId: "recipe01", requestedCount: 1n, requestKey: "building" }), /BUILDING_EXECUTION_REQUIRES_HOME_PROVIDER/);
