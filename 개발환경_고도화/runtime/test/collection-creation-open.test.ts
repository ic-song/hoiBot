...
import {
  CollectionCreationOpenService,
  isCollectionCreationOpenCommand
} from "../src/mini-pet/collection-creation-open-service.js";
import { ApplicationError } from "../src/shared/application-error.js";

// 컬렉션 창조 오픈 SQL 순서와 mutation 유무를 기록하는 테스트 DB를 만듭니다.
...
const items = [
  { item_id: 51n, code: "bag_11319869697c3c00", quantity: 1n, version: 2n },
  { item_id: 52n, code: "bag_3241894752b82f7a", quantity: 10n, version: 3n }
];
const successMessage = "🐹 컬렉션창조패키지 오픈 완료!\n\n🎁 지급: [컬렉션창조 미니펫🐹] (창조 / 매력+1💕)\n🎟️ 추가지급: 미니펫뽑기🐹(/미니펫오픈) 2000개\n\n👉 /미니펫가방 으로 확인해주세요.";

describe("collection creation open policy", () => {
  it("accepts the exact no-argument command only", () => {
    assert.equal(isCollectionCreationOpenCommand("/컬렉션창조오픈"), true);
    for (const message of ["/컬렉션창조오픈 ", "/컬렉션창조오픈 1", "/컬렉션창조오픈 안내", "/컬렉션창조오픈추가"]) {
      assert.equal(isCollectionCreationOpenCommand(message), false);
    }
  });
});

describe("collection creation open service", () => {
  it("keeps the legacy silent block during an active castle siege", async () => {
    ...
    const result = await new CollectionCreationOpenService(scripted.database).handle({
      externalUserId: "kakao-11", channelId: "room-1", message: "/컬렉션창조오픈", eventId: "event-siege"
    });
  ...
  it("keeps the legacy silent return for an unregistered sender", async () => {
    ...
    const result = await new CollectionCreationOpenService(scripted.database).handle({
      externalUserId: "unknown", channelId: "room-1", message: "/컬렉션창조오픈", eventId: "event-unknown"
    });
  ...
  it("blocks a full mini-pet bag before reading or mutating inventory", async () => {
    ...
    await assert.rejects(
      () => new CollectionCreationOpenService(scripted.database).handle({
        externalUserId: "kakao-11", channelId: "room-1", message: "/컬렉션창조오픈", eventId: "event-full"
      }),
  ...
  it("reports the legacy package shortage without mutation", async () => {
    ...
    await assert.rejects(
      () => new CollectionCreationOpenService(scripted.database).handle({
        externalUserId: "kakao-11", channelId: "room-1", message: "/컬렉션창조오픈", eventId: "event-short"
      }),
      (error: unknown) => error instanceof ApplicationError && error.code === "COLLECTION_CREATION_PACKAGE_REQUIRED"
        && error.message === "❌ [🌱합성회원 남] 님\n컬렉션창조패키지🐹(/컬렉션창조오픈) 아이템이 없습니다.\n(보유: 0개)"
    );
    assert.equal(scripted.sql.some((statement) => statement.startsWith("UPDATE ") || statement.startsWith("INSERT ")), false);
  });
  ...
  it("consumes the package and grants ticket and mini pet atomically", async () => {
    ...
    const result = await new CollectionCreationOpenService(scripted.database).handle({
      externalUserId: "kakao-11", channelId: "room-1", message: "/컬렉션창조오픈", eventId: "event-open"
    });
    assert.deepEqual(
      { status: result.status, package: result.packageQuantity, ticket: result.ticketQuantity, data: result.data },
      { status: "opened", package: "0", ticket: "2010", data: successMessage }
    );
  ...
  it("creates a missing ticket stack on first reward", async () => {
    ...
    const result = await new CollectionCreationOpenService(scripted.database).handle({
      externalUserId: "kakao-11", channelId: "room-1", message: "/컬렉션창조오픈", eventId: "event-first-ticket"
    });
    assert.equal(result.ticketQuantity, "2000");
  ...
  it("returns the stored result on a repeated event without mutation", async () => {
    const storedResult = { status: "opened" as const, playerId: "21", packageQuantity: "0", ticketQuantity: "2000" };
    const scripted = createScriptedDatabase([[{ active_count: 0n }], [owner], [{ result_json: JSON.stringify(storedResult) }]]);
    const result = await new CollectionCreationOpenService(scripted.database).handle({
      externalUserId: "kakao-11", channelId: "room-1", message: "/컬렉션창조오픈", eventId: "event-repeat"
    });
