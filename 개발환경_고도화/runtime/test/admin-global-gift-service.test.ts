...
describe("admin global gift canonical reintegration",()=>{
  ...
  it("uses canonical CUID ownership, snapshot receipts and no operational room seed",()=>{
    ...
    assert.doesNotMatch(migration,/canonical_admin_global_gift_migration_provenance|catalog_seed_created_flag/);
    assert.doesNotMatch(rollback,/DELETE FROM canonical_item_definition_imports/);
    assert.doesNotMatch(rollback,/DELETE FROM canonical_item_definitions/);
    assert.doesNotMatch(rollback,/INFORMATION_SCHEMA\.KEY_COLUMN_USAGE|GROUP_CONCAT/);
    ...
    assert.match(rollback,/ADMIN_GLOBAL_GIFT_DURABLE_STATE_EXISTS/);
