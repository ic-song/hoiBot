...
describe("object DB executable parity Wave17 item-bag read providers",()=>{
  it("freezes exactly five direct READ scenarios per WBS776 SQL consumer",()=>{
    ...
    assert.deepEqual(fixture.consumerIds,["sql-repository-19f17500144188bf","sql-repository-2415b4267e1577c6","sql-repository-3b23c2f0f5501988"]);assert.equal(fixture.bindings.length,15);
    ...
    assert.ok(fixture.runtimeSourcePaths.includes("개발환경_고도화/runtime/src/inventory/legacy-rank-label-side-effect-readiness-provider.ts"));
  ...
  it("rejects target tampering and requires a committed evidence SHA for generation",()=>{
    ...
    const generator=read("개발환경_고도화/runtime/scripts/generate-object-db-consumer-executable-parity-wave17-item-bag-read-providers.ts");assert.match(generator,/Wave17 exact 40-character evidenceCommit required/);assert.match(generator,/\.\.\.prior\.receipts,\.\.\.added/);assert.match(generator,/receipts\.length!==182/);assert.doesNotMatch(generator,/wave6-multi-query|receipt:wave6:/);
  });
});
