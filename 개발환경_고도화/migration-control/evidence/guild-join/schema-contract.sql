-- CMD-04-0001 domain draft only. Coordinator must allocate the final migration number.
ALTER TABLE guilds
  ADD COLUMN mark VARCHAR(32) NOT NULL DEFAULT '' AFTER display_name,
  ADD COLUMN server_code VARCHAR(64) CHARACTER SET ascii COLLATE ascii_bin NOT NULL AFTER mark,
  ...
  ADD CONSTRAINT fk_guild_server FOREIGN KEY (server_code) REFERENCES game_servers (code) ON DELETE RESTRICT;
...
-- The shared synthetic fixture must define one active stackable item with code legacy-guild-join-ticket.
-- recruitment_bonus is a projection refreshed from the equipped guild-master skill code legacy-recruitment-command.
