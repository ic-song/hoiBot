...
| `legacy-6a42c658dfc99e35` | 컬렉션 순위+정규화 | READ_WRITE | public / repair root if needed |
| `legacy-7e211a21f6194155` | 대기 등록 취소 | READ | self session / DB transaction 없음 |
| `legacy-ae36b9a9e188508d` | 컬렉션 조회+정규화 | READ_WRITE | self / repair root if needed |
...
| `sql-repository-649707c7a4a4b178` | `mini-pet.registerDefinition` | READ_WRITE | repository root |
| `sql-repository-81ba01f732909dc7` | `mini-pet.updateDefinition` | READ_WRITE | repository root |
...
- 독립 title mutation은 `canonical_mini_pet_title_operations`가 replay receipt를 소유한다. 관리자 교차 사용자 작업은 participant에 `OWNER`, `RECIPIENT`를 기록한다.
...
- P1: repository mutation 5개가 canonical operation/participant receipt를 기록하지 않음.
