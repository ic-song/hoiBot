...
- `ADMIN_PET_TITLE_SYNC`는 WBS746이 승인한 active-member authority와 exact 대상 집합을 제공하기 전까지 `ACTIVE`에서도 `LEGACY_FALLBACK`으로 고정합니다. 기존 Node `adminSync` mutation을 다시 호출하지 않으며 dual write를 허용하지 않습니다.
...
- WBS746 active-member authority 연결 뒤 `ADMIN_PET_TITLE_SYNC` MODERN 구현·검증
- Gate 5~7 및 운영 cutover

현재 변경은 격리 개발 worktree와 테스트 DB에만 있으며 커밋·push·`feature/prod` 반영을 하지 않았습니다.
