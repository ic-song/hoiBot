...
4. 빈 값·로딩·오류·재시도·live announcement, visible focus, reduced-motion CSS, 375px 모바일 재배치를 source와 focused test로 검증했다.

## 실행 결과

2026-09-10에 runtime 작업 디렉터리에서 `node --import tsx --test test/user-shell.test.ts test/site-web-app-wiring.test.ts`를 실행해 9/9 통과했다. `npm run typecheck`, `npm run build`, `git diff --check`도 통과했다. 작업반장 브라우저 검증은 `foreman-visual-qa.md`에 분리해 기록했다. API 등록/provider/app.ts 통합은 Lease2642 종료 후 수행해야 하므로 Gate 7 및 Gate 8 근거는 이 UI evidence에 포함하지 않는다.
