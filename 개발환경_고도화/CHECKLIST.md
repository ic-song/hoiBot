...
## 2026-07-30 개발 PC 선행 검증

운영 PC 체크리스트와 구분해 개발 PC의 LDPlayer 9에서 먼저 확인한 결과다. 상세 내용은 `IRIS_TEST_RESULTS_20260730.md`를 참고한다.

- [x] 별도 테스트방 메시지를 Iris가 감지했다.
- [x] Iris HTTP endpoint가 개발 PC Lite 서버로 이벤트를 전달했다.
- [x] `msg`, `room`, `sender`, `json.chat_id`, `json.user_id` 등 기본 필드를 확인했다.
- [x] 연속 메시지 3건이 별도 이벤트로 전달되고 중복 HTTP 전달이 없음을 표본에서 확인했다.
- [x] 메시지 삭제 이벤트의 식별 필드와 원본 연결을 확인했다.
- [x] health, ping, 인증, body 제한, 최근 이벤트 저장 자동 테스트를 구성했다.
- [ ] 운영 PC에서 동일한 결과를 재검증한다.
- [ ] Iris `/reply`와 WebSocket `/ws`를 검증한다.
