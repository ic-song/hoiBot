...
| `feature/hoi` | hoi 작업용 브랜치입니다. `tools/`를 통한 업로드와 운영 반영 전 작업 브랜치로 사용하며, `feature/prod`에서 분기합니다. |
...
```text
1. 최초 1회 `/패키지초기화`로 기본 패키지/로그 파일 생성
2. `/패키지추가방법`으로 입력 형식 확인
3. 단계형은 `/패키지추가시작` 입력 후 안내에 따라 진행
4. 빠른 등록은 `/패키지추가 패키지명 | 설명 | 보상목록` 입력
5. `/패키지리스트`에서 새 번호 확인
6. 지급은 `/패키지지급 이름 리스트번호 갯수`로 처리
```

예시:

```text
/패키지추가 이벤트패키지🎁 | 이벤트 보상 패키지 | point:10000000, item:펫 강화석⭐:10
```
...
패키지명 하나만 저장하며, 가방에도 같은 이름으로 표시됩니다. 별도 가방 아이템명은 사용하지 않습니다.
