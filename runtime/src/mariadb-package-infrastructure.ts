...
export class MariaDbPackageCatalogRepository implements PackageCatalogRepository, PackageRewardRuleRepository, PackageUseOperationRepository {
  ...
  async listDynamicRewardItems(
    ...
    if (catalogCode !== undefined) {
      ...
      const rows = await connectionOf({ transactionHandle: transaction.handle } as ItemMutationContext).query<SqlRow[]>(
        "SELECT e.item_id, e.grade_code, e.grade_weight, e.item_weight, d.item_type, d.metadata_json FROM dynamic_item_catalog_entries e JOIN item_definitions d ON d.item_id = e.item_id WHERE e.catalog_code = ? AND e.enabled = 1 AND d.enabled = 1 ORDER BY e.grade_code, e.item_id FOR UPDATE",
        [catalogCode]
      );
      return rows.map((row) => ({
        itemId: String(row.item_id),
    ...
    const rows = await connectionOf({ transactionHandle: transaction.handle } as ItemMutationContext).query<SqlRow[]>(
      "SELECT item_id, item_type, metadata_json FROM item_definitions WHERE enabled = 1 AND item_type = ? ORDER BY item_id FOR UPDATE",
      [itemType]
    );
    return rows.map((row) => ({
      itemId: String(row.item_id),
  ...
  async getRemainingItemCapacity(
    ...
    const activeRows = await db.query<SqlRow[]>(
      "SELECT i.instance_id FROM item_instances i JOIN item_definitions d ON d.item_id = i.item_id WHERE i.owner_type = ? AND i.owner_id = ? AND d.item_type = ? AND i.removed_at IS NULL ORDER BY i.instance_id FOR UPDATE",
      [ownerType, ownerId, itemType]
    );
...
export class MariaDbItemDefinitionRepository implements ItemDefinitionRepository {
  ...
  async findById(itemId: string, transactionHandle?: unknown): Promise<ItemDefinition | undefined> {
    ...
    const rows = await db.query<SqlRow[]>(
      "SELECT item_id, item_type, item_name, stackable, metadata_json, enabled FROM item_definitions WHERE item_id = ?" + (transactionHandle ? " FOR UPDATE" : ""),
      [itemId]
    );
    const row = rows[0];
    return row ? {
...
export class MariaDbStackItemHandler implements ItemTypeHandler {
  ...
  async checkRemove(definition: ItemDefinition, quantity: bigint, context: ItemMutationContext): Promise<void> {
    ...
    const rows = await connectionOf(context).query<SqlRow[]>(
      "SELECT quantity FROM item_balances WHERE owner_type = ? AND owner_id = ? AND item_id = ? FOR UPDATE",
  ...
  async add(definition: ItemDefinition, quantity: bigint, context: ItemMutationContext): Promise<void> {
    ...
    await this.assertAddCapacity(definition, quantity, context);
    await db.query(
      "INSERT INTO item_balances(owner_type, owner_id, item_id, quantity) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE quantity = quantity + VALUES(quantity), row_version = row_version + 1",
  ...
  async remove(definition: ItemDefinition, quantity: bigint, context: ItemMutationContext): Promise<void> {
    ...
    const result = await db.query(
      "UPDATE item_balances SET quantity = quantity - ?, row_version = row_version + 1 WHERE owner_type = ? AND owner_id = ? AND item_id = ? AND quantity >= ?",
  ...
  private async ledger(db: PoolConnection, itemId: string, delta: bigint, context: ItemMutationContext): Promise<void> {
    await db.query(
      "INSERT INTO item_ledger(ledger_id, operation_id, owner_type, owner_id, item_id, quantity_delta) VALUES (?, ?, ?, ?, ?, ?)",
...
export class MariaDbInstanceItemHandler implements ItemTypeHandler {
  async checkAdd(definition: ItemDefinition, quantity: bigint, context: ItemMutationContext): Promise<void> {
    ...
    if (definition.type === "PET_APPEARANCE") {
      ...
      await connectionOf(context).query(
        "SELECT instance_id FROM item_instances WHERE owner_type = ? AND owner_id = ? AND target_instance_id = ? AND removed_at IS NULL AND item_id IN (SELECT item_id FROM item_definitions WHERE item_type = 'PET_APPEARANCE') FOR UPDATE",
  ...
  async checkRemove(definition: ItemDefinition, quantity: bigint, context: ItemMutationContext): Promise<void> {
    ...
    const rows = await connectionOf(context).query<SqlRow[]>(
      "SELECT COUNT(*) AS count FROM item_instances WHERE owner_type = ? AND owner_id = ? AND item_id = ? AND removed_at IS NULL FOR UPDATE",
      [context.ownerType, context.ownerId, definition.id]
    );
  ...
  async add(definition: ItemDefinition, quantity: bigint, context: ItemMutationContext): Promise<void> {
    ...
    if (definition.type === "PET_APPEARANCE") {
      ...
      const previous = await db.query<SqlRow[]>(
        "SELECT instance_id, item_id FROM item_instances WHERE owner_type = ? AND owner_id = ? AND target_instance_id = ? AND removed_at IS NULL AND item_id IN (SELECT item_id FROM item_definitions WHERE item_type = 'PET_APPEARANCE') FOR UPDATE",
        [context.ownerType, context.ownerId, context.targetSelector]
      );
      for (const row of previous) {
        await db.query("UPDATE item_instances SET removed_at = CURRENT_TIMESTAMP(3) WHERE instance_id = ?", [row.instance_id]);
        await db.query(
          "INSERT INTO item_ledger(ledger_id, operation_id, owner_type, owner_id, item_id, quantity_delta, instance_id) VALUES (?, ?, ?, ?, ?, -1, ?)",
    ...
    for (let i = 0n; i < quantity; i++) {
      ...
      await db.query(
        "INSERT INTO item_instances(instance_id, owner_type, owner_id, item_id, target_instance_id, instance_metadata_json) VALUES (?, ?, ?, ?, ?, ?)",
  ...
  async remove(definition: ItemDefinition, quantity: bigint, context: ItemMutationContext): Promise<void> {
    ...
    const rows = await db.query<SqlRow[]>(
      "SELECT instance_id FROM item_instances WHERE owner_type = ? AND owner_id = ? AND item_id = ? AND removed_at IS NULL ORDER BY created_at LIMIT ? FOR UPDATE",
  ...
  private async validatePetTarget(definition: ItemDefinition, context: ItemMutationContext): Promise<void> {
    ...
    const rows = await connectionOf(context).query<SqlRow[]>(
      "SELECT i.instance_id FROM item_instances i JOIN item_definitions d ON d.item_id = i.item_id WHERE i.instance_id = ? AND i.owner_type = 'USER' AND i.owner_id = ? AND i.removed_at IS NULL AND d.item_type IN ('PET', 'MINI_PET') FOR UPDATE",
