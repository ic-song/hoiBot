...
- 캐슬대전 경험치는 승리 50 EXP, 패배 25 EXP이며 호월신의 가호와 티어 보너스, 레벨 상승 처리를 공통 경험치 흐름으로 적용한다.
...
# !알림내용|!알림시작|!알림초기화|!알림정보|/알림리셋

Status: VERIFIED

## Command Anchors

- Search in `notice.js`: `!알림내용`, `!알림시작`, `!알림초기화`, `!알림정보`, `/알림리셋`
...
- `/알림리셋`은 실행 타이머를 해제하고 저장된 알림 문구·예약 상태·실행 토큰·다음 발송 시각을 초기화한 뒤 독립 데이터 파일을 한 번 저장한다.
...
- `/알림리셋`은 저장된 알림 문구를 삭제하고 실행 중인 반복 예약을 중지한다.
...
- 미니펫대전 경험치는 승리 22.5 EXP, 패배 7.5 EXP이며 계산 중 발생한 EXP 소수는 셋째 자리에서 반올림한다. 호월신의 가호와 티어 보너스, 레벨 상승 처리는 공통 경험치 흐름을 사용하고 소수 EXP에 필요한 가호 수량은 개 단위로 올림한다.
...
- `/퀘스트` 안내에는 일일 100 EXP, 주간 500 EXP, 프리미엄 전용 일퀘 50 EXP를 각 보상 영역에 표시한다. 완료 시 호월신의 가호 3배를 먼저 적용한 뒤 티어 경험치 보너스를 계산하며, 실제 총 지급량과 가호 추가분·기본·티어 내역을 표시한다. 가호 소진과 레벨업도 기존 공통 흐름으로 처리한다.
...
- 추석 이벤트가 활성화되면 일반 탐험 선택·정산을 일시 중지하고 길드 가입 유저의 `/탐 11`만 받는다. 자동탐험권 이용자는 기존 자동고정 번호와 관계없이 정각 처리 전에 11번으로 자동 등록되며 `/자동탐고정 11` 설정은 필요하지 않다. 매시간 정각 현재 길드 가입과 1억 포인트 보유를 다시 확인하며, 실제 참가자는 1억 포인트를 내고 기존 유저별 성공률과 확률UP 로직을 적용한다. 성공하면 황금당근 1~3개를 같은 확률로 받고 실패하면 황금당근을 받지 않는다.
- `/탐 11` 예약 화면은 상세 성공률을 표시한다. 이벤트 `/지도`는 자동탐험권 보유자의 달토끼 자동 참여를 안내하고, 기본·티어·매력·영주·펫스킬·펜던트·호이패스·홈뱃지·확률UP·디버프의 상세 성공률을 `allsee` 뒤에 표시한다.
...
- `/탐 11`은 길드 가입 여부와 1억 포인트 보유 여부를 선택 시점에 먼저 확인하며, 조건을 충족하지 못하면 탐험지에 등록하지 않는다.
...
- 펫탐험은 성공·실패와 관계없이 실제 정산까지 완료한 계정에 일반 광산 3 EXP, 일반·무작위 이벤트 던전 5 EXP, 미궁 7 EXP를 지급한다. 다이아 광산(`/탐 0`), 길드레이드던전(`/탐 10`), 달토끼 탐색(`/탐 11`)은 각각 10 EXP를 지급한다. 입장 실패·미참여에는 지급하지 않으며, 입장권 부족으로 광산 이동이 발생하면 최종 실제 탐험지 종류를 기준으로 한다. 경험치에는 호월신의 가호 3배와 현재 티어 보너스를 순서대로 반영한다.
- 펫탐험 정산 메시지는 실제 기본·가호·티어 합산값을 `📊 총 경험치`로 표시한다. 레벨업·승급·대승급 안내는 결과 보고서의 `allsee` 앞에 별도 표시해 접힌 상세 결과에 가려지지 않게 한다.
- 달토끼 탐색 결과는 성공한 참가자를 먼저 표시하고 실패한 참가자를 그 뒤에 표시한다. 각 그룹 안에서는 기존 참가 처리 순서를 유지하며, 상단의 미참여·길드 미가입·포인트 부족 인원 요약 아래에 길드 미가입 유저와 포인트 부족 유저의 닉네임을 각각 표시한 뒤 참가자 상세용 `allsee`를 연결한다.
- 달토끼 탐색은 매 정각 현재 길드 가입과 참가비 보유 조건을 다시 확인하고, 같은 정각의 자동 중복 차감·보상과 0명 결과 재출력을 막는다. 포인트 부족자는 처리 완료로 기록하지 않아 충전 후 다음 정각에 참여할 수 있다. 관리자 수동 `/펫탐험정산`은 DEV·운영 모두 입력할 때마다 새로운 탐험 회차로 실행한다.
...
- `/자동탐험시작`은 기존 타이머를 정리하고 명령 입력 시각을 자동 주기 기준으로 저장한 뒤 첫 자동 정산을 60분 후 실행한다. 시작 명령에서는 정산·출발 대기를 수행하지 않고 즉시 종료하며, 관리자 `/펫탐험정산`은 즉시 정산만 수행하고 자동 주기 기준 시각을 바꾸지 않는다. 이후 타이머는 별도 채팅 입력을 기다리지 않고 정산 응답을 직접 실행하며, DEV에서 시작한 타이머는 DEV 데이터 경로를 유지한다.
...
- 기존 가호 적용 대상인 일반 채팅·캐슬대전·미니펫대전·펫탐험은 가호가 1개 이상 남아 있으면 해당 정산의 기본 EXP 전체를 3배로 지급한다. 가호는 3배 적용된 기본 EXP 수량만큼 최대 보유량까지 차감하며, 필요량보다 적게 남은 마지막 정산도 전체 3배를 지급한 뒤 0개로 만든다. 소진 안내에는 후원 링크를 표시하지 않는다.
