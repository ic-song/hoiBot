...
- `개발환경_고도화/runtime/migrations/037_open_all_guild_resources.sql`
...
- Modernization: `item_definitions`, `inventory_stacks`, `inventory_ledger`, `currency_accounts`, `currency_ledger`, `player_counters`
- Modernization guild: `guilds`, `guild_members`, `guild_resource_accounts`, `guild_resource_ledger`, `guild_warehouse_stacks`, `guild_warehouse_ledger`
...
- The modernization path locks inventory/currency before guild/member/resource/warehouse/member-food state and commits item/point/guild ledgers, execution, audit, and ordered outbox in one MariaDB transaction.
...
- Guild contribution medals and guild warehouse packages are included in `SL-INVENTORY-OPEN-ALL`; standalone guild contribution/shop commands remain outside this slice.
- Guild level-up rewards preserve the current cumulative threshold loop, including the actor and other guild members in all-member pet-food grants.
