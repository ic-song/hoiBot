...
- 자동일퀘 결과의 접히지 않는 영역에 이번 실행에서 실제 지급한 레벨업 포인트와 레벨업 횟수를 별도로 표시한다. 총 포인트 변동은 기존처럼 모든 획득·사용을 합산한 순변동이다.
...
- 추석 이벤트가 활성화되면 일반 탐험 선택·정산을 일시 중지하고 길드 가입 유저의 `/탐 11`만 받는다. `/자동탐고정 11`을 설정한 자동탐험권 이용자는 정각 처리 전에 11번으로 자동 등록된다. 매시간 정각 현재 길드 가입과 1억 포인트 보유를 다시 확인하며, 실제 참가자는 1억 포인트를 내고 기존 유저별 성공률과 확률UP 로직을 적용한다. 성공하면 황금당근 1~3개를 같은 확률로 받고 실패하면 황금당근을 받지 않는다.
- `/탐 11` 예약 화면은 상세 성공률을 표시한다. 이벤트 `/지도`는 자동탐험 고정 상태와 `/자동탐고정 11` 안내를 표시하고, 기본·티어·매력·영주·펫스킬·펜던트·호이패스·홈뱃지·확률UP·디버프의 상세 성공률을 `allsee` 뒤에 표시한다.
...
- 달토끼 탐색 결과는 성공한 참가자를 먼저 표시하고 실패한 참가자를 그 뒤에 표시한다. 각 그룹 안에서는 기존 참가 처리 순서를 유지하며, 미참여자는 길드 미가입·포인트 부족 인원수만 상단에 요약한다.
- 달토끼 탐색의 정각 처리 완료값은 길드 가입과 참가비 보유 조건을 모두 통과한 실제 참여자에게만 기록한다. 포인트 부족으로 미참여한 사용자는 포인트 충전 후 같은 시간대 재정산에서도 참여할 수 있다.
- 달토끼 탐색의 시작 알림과 지도 참여 인원은 현재 길드 가입·참가비 조건을 충족한 등록자만 집계한다. 참여자가 0명이면 접힌 빈 상세 결과를 붙이지 않고, 탐험 선택 유지와 다음 정각 참여 조건을 안내한다.
