...
- `/맞짱시간체크 [닉네임]` loads `homeDataFile` and measures the named user's `calculateTotalExp` runtime
...
- `/맞짱시간체크 [닉네임]` measures only the named user's 종합매력 calculation and reports response-entry total, diagnostic-branch, home-load, and 종합매력 calculation timings; it does not select an opponent or run `runMatzangBattle`
