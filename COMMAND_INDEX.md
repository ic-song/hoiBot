...
- `/펫홈활동파일생성`: Admin/Master-only exact command; the modern runtime records the relational activity store bootstrap state only when absent, detects and preserves pre-existing activity rows, and never clears alerts, recent visitors, social, badge, or migration projections. Legacy Rhino behavior remains frozen.
- `/펫홈활동살리기`: Admin/Master-only exact command; the modern runtime validates the relational `petHomeActivityData_back.json` staging rows, preserves the source and current projections, then atomically replaces alerts, recent visitors, social relations, heart usage, home badges, activity dates, special logs, and migration markers. Legacy Rhino behavior remains frozen.
...
- `/댓글핀 [번호]`의 현대화 handler는 `HOME_COMMENT_PIN`이며, 활성 패스·펫·최신순 stable comment ID·중복·최대 3개를 재확인하고 핀·홈 version·감사·outbox·재실행 결과를 한 transaction에 기록한다.
...
- 현대화 조회 provider는 migration 374의 `v2.400` 불변 definition version을 고정해 204종 순서와 HB 57개·MBTI 20개·LOVE 50개의 이름·이모지·설명·등급을 원문 그대로 사용한다. 기존 migration 322의 placeholder version은 이력으로 보존하고 retired 처리한다.
...
- 현대화 SHADOW provider는 migration 375의 DB 정책으로 세 티켓과 C/B/A/S `55/30/12/3`, 중복당 1억 포인트를 관리하고, definition version `930000002`를 고정한다. event·player·variant·count 기반 결정적 RNG trace, draw 순서별 신규/중복/영구삭제 판정, 티켓·포인트 원장, 결과 reply 뒤 S등급 공지를 하나의 transaction과 ordered outbox로 기록한다.
...
- 현대화 `/홈뱃지큐브` SHADOW provider는 migration 376의 DB 옵션·재고·51개 확률 구간을 사용한다. definition version `930000002`와 cube config version `930000003`을 고정하고, 시도별 2단계 결정적 RNG trace, 정수 보호선, 실제 사용량 원장, 최초 10% 단계·all-max 공지를 하나의 transaction과 ordered outbox로 기록한다.
...
- 대회 진행 중 일반 유저는 `main.js`에서 `/펫무쌍공격 [1-10]`, `/무쌍순위`(모집 중 `/펫무쌍준비` 추가 허용) 외의 슬래시 명령과 무슬래시 입력이 차단되며, 운영자는 관리 명령을 계속 사용할 수 있다. `Info.js`의 정보 명령도 일반 유저에게 차단하며, 제한 안내는 `main.js`에서 한 번 출력한다. 단, 펫무쌍·맞짱필드·길드 영지전은 어느 시작 경로에서도 동시에 활성화되지 않도록 상호 차단한다.
...
## Save Flow
- `/봉인금고오픈 [숫자]`는 요청 횟수만큼 `호이의 봉인금고🔒(/봉인금고오픈 숫자)`와 `해방의 열쇠🗝️(/봉인금고오픈 숫자)`를 모두 보유한 회원만 실행할 수 있다.
- `/봉인금고오픈 [숫자]`는 1~100개의 봉인금고와 열쇠를 한 번에 사용하며, 각 회차를 순서대로 추첨한 뒤 두 아이템 차감·보상·누적 상태·희귀 기록을 `member.json`에 한 번 저장한다.
...
- `/해방열쇠조합 [숫자]`는 열쇠 1개당 봉인금고 100개를 소모한다. 성공 결과에 사용자 체크랭크, 소모·획득 수량, 남은 호봉과 현재 열쇠 수량을 표시한다. `runSealedVaultKeyCombine`은 메모리만 변경하고 기존 명령 진입부에서 `member.json`에 한 번 저장한다. 재료 부족·0·계산 범위 초과는 차감하지 않고 후행 안내문이 붙은 입력은 실행하지 않는다.
- `해방열쇠조합` 또는 `/해방열쇠조합`은 사용법만 안내한다. 열쇠 부족 안내에 조합 안내를 추가하고 봉인금고 부족 안내는 일일퀘스트 획득 경로를 표시한다.
- 일일퀘스트 보상은 `claimQuestReward`의 기존 중복 수령 방지와 저장 경로를 재사용하며 main·Info 퀘스트 창과 지급 결과에 봉인금고 1개를 표시한다.
...
- 현대화 `/디버깅모드` handler는 `ADMIN_DEBUG_MODE`이며, 프로세스별 UUID 상태로 재시작 시 OFF를 보존하고 RBAC·재실행 결과·감사·outbox를 한 transaction에 기록한다. 이후 현대화 진단 소비처는 `DebugModeService.isEnabled()`를 공용 provider로 사용한다.
...
- `data.member[*].bag[입력한 아이템이름]`
- `data.member[*].pass.premium`

## Save Flow

- `/선물전달 아이템이름/갯수`는 전체 가입자의 가방에 입력 아이템을 지급한 뒤 `filePath`를 한 번 저장한다.
- `/프리미엄전달 아이템이름/갯수`는 현재 활성 호이패스 프리미엄 이용자만 선별해 입력 아이템을 지급한 뒤 `filePath`를 한 번 저장한다.

## AI Notes

- 사용 권한은 `호이 남`으로 유지한다.
- 인자 없는 `/선물전달`은 사용법만 출력하며 지급하지 않는다.
- 인자 없는 `/프리미엄전달`도 사용법만 출력하며 지급하지 않는다.
- 아이템명 내부의 `/관련명령어` 표기는 유지하고 마지막 `/숫자`만 지급 수량으로 분리한다.
- 두 명령 모두 지급 완료 후 `noticeMsg`로 전체 공지를 보내고, 명령어를 실행한 채팅방에는 지급 대상 인원을 포함한 완료 메시지를 한 번 별도로 출력한다.
- 전체 공지는 `🎁 호이가 여러분의 주머니에`로 시작하는 선물 안내 문구와 실제 지급 아이템·수량, `/패키지가방 을 확인해주세요.` 안내를 함께 표시한다.
- `호이의 봉인금고🔒(/봉인금고오픈 숫자)`처럼 명령어 표기가 포함된 공식 아이템명도 그대로 지급할 수 있다.
- `/프리미엄전달`은 활성 프리미엄 이용자가 없으면 지급·저장·전체 공지를 실행하지 않는다.
- 프리미엄 지급 완료 공지는 `noticeMsg`로 호이패스 프리미엄 전용 문구와 실제 지급 아이템·수량, `/패키지가방` 확인 안내까지만 표시한다.
