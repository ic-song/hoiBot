...
- `/해방열쇠조합 [숫자]`는 열쇠 1개당 봉인금고 100개를 소모한다. `runSealedVaultKeyCombine`은 메모리만 변경하고 기존 명령 진입부에서 `member.json`에 한 번 저장한다. 재료 부족·0·계산 범위 초과는 차감하지 않고 후행 안내문이 붙은 입력은 실행하지 않는다.
