...
- 추석 이벤트가 활성화되면 일반 탐험 선택·정산을 일시 중지하고 길드 가입 유저의 `/탐 11`만 받는다. 수동 `/탐 11` 등록은 다음 정산 1회 후 초기화된다. 자동탐험권 이용자는 기존 자동고정 번호와 관계없이 정각 처리 전과 정산 후 다음 회차에 11번으로 자동 등록되며 `/자동탐고정 11` 설정은 필요하지 않다. 매시간 정각 현재 길드 가입과 5천만 포인트 보유를 다시 확인하며, 실제 참가자는 5천만 포인트를 내고 기존 유저별 성공률과 확률UP 로직을 적용한다. 성공하면 황금당근 1~3개를 같은 확률로 받고 실패하면 황금당근을 받지 않는다.
- `/탐 11` 예약 화면은 상세 성공률을 표시한다. 이벤트 `/지도`는 저장된 수동 참여자와 자동탐험권 보유자를 합쳐 다음 정각의 현재 참여 인원을 계산하고, 자동탐험권 신규 획득자도 저장 등록 전부터 달토끼 자동 참여 예정으로 표시한다. 기본·티어·매력·영주·펫스킬·펜던트·호이패스·홈뱃지·확률UP·디버프의 상세 성공률은 `allsee` 뒤에 표시한다.
...
- `/자동탐고정해제`는 저장된 자동탐험 고정 번호만 삭제하며 일반 `/지도`의 자동탐험 고정 안내에도 함께 표시한다. 현재 탐험 참여 상태와 자동탐험권은 유지하며, 추석 이벤트 중에는 자동탐험권 보유자의 11번 자동 참여가 계속 적용된다.
