...
- `/펫정보`의 펫강화 줄은 대표 홈뱃지의 펫강화 큐브 옵션이 활성일 때 저장 강화수치 → 반올림된 유효 강화수치와 적용 퍼센트를 표시한다. 치명타는 유효 강화수치를 사용하지만 강화 성공확률은 변경하지 않는다.
