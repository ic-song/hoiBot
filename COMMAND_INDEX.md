...
- `/자동탐험시작`은 기존 타이머를 정리하고 입력 즉시 새로운 펫탐험 회차를 1회 정산한 뒤 60분마다 정산 응답을 직접 실행한다. 달토끼 탐색의 같은 시간 처리 기록이 있어도 시작 명령의 즉시 정산은 생략하지 않으며, 별도 채팅 입력을 기다리지 않고 DEV에서 시작한 타이머는 DEV 데이터 경로를 유지한다.
