...
- `/홈뱃지큐브 [번호] [옵션] [횟수]` is available without a pass and uses 1 cube per castle/raid attempt, 2 per pet-upgrade attempt, and 3 per explore attempt. It allows 1–100 tries and stops at the option maximum or cube shortage. A higher roll applies its full decimal value immediately and may skip multiple integer bands. Decimal values can move within the current band, while only the highest reached integer 1% floor is protected: 2.3% can jump to 22.5%, then move to 22.2%, and a later 21.7% roll is protected at 22.0%. The high-value tail is balanced so 300 independent rolls reach about 8.3% for 27%+, 5.8% for 30%+, and 1.8% for 40%+. Maximums are castle 50%, raid 50%, pet-upgrade 30%, and explore 15%. Each option announces every first-time 10% milestone crossed by a jump. When the four base values total at least 100%, the equipped badge applies `×1.1` to every option. Option 3 multiplies the stored pet-upgrade level by its applied percentage and rounds to the nearest integer for critical and total-charm calculations; it does not change upgrade success probability, cost, or the stored level. While `/디버깅모드` is ON, successful `/홈뱃지장착` sends the test room a before/after comparison for castle/raid charm, effective pet-upgrade level, and the current exploration chance. `/큐브확률` is exact/read-only, available without a pass, and shows the configured range rates to six decimal places.
