...
- `isDuplicatePendantCreationCombination`
- `markPendantCreationCombination`
...
- `/펜던트조합창조`는 선택한 기본 창세 펜던트 3개를 모두 검증한 뒤 가방에서 제거하고 기본 창조 펜던트 1개를 추가해 `member_pet.json`을 저장한다. 저장 성공 뒤에만 완료 응답을 보내며 동일 요청의 즉시 중복 처리를 막는다.
...
- `/펜던트조합창조`는 서로 다른 양의 가방 번호 3개를 받고 장착되지 않은 무강화 창세 펜던트만 재료로 사용한다. 포인트와 다른 아이템은 소모하지 않고 100% 확률로 기본 창조 펜던트 1개를 가방에 넣는다.
